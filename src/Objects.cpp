...
void Objects::SphereCheckPossible(XYZ* p1, float radius)
{
    ...
    if(whichpatchx >= 0 && whichpatchz >= 0 && whichpatchx < subdivision && whichpatchz < subdivision)
        if(terrain.patchobjectnum[whichpatchx][whichpatchz] > 0 &&
           terrain.patchobjectnum[whichpatchx][whichpatchz] < 500)
...
void Objects::Draw()
{
    ...
    for(i = 0; i < numobjects; i++) {
        if(type[i] != firetype) {
            ...
            if(type[i] == tunneltype ||
               frustum.SphereInFrustum(position[i].x + moved.x,
                                       position[i].y + moved.y,
                                       position[i].z + moved.z,
                ...
                hidden = !(findDistancefastflat(&viewer, &position[i]) > playerdist + 3 ||
                           (type[i] != bushtype && type[i] != treeleavestype));
                if(!hidden) {

                    if(config.video_detail == 2 && distance > viewdistance * viewdistance / 4 && environment == desertenvironment)
                    ...
                    if(distance > 0) {
                        ...
                        if(occluded[i] < 6) {
                            ...
                            if(type[i] == bushtype) {
                                ...
                                if(rotxvel[i] || rotx[i]) {
                                    if(rotx[i] > 0)
                                        rotxvel[i] -= multiplier * 8 * fabs(rotx[i]);
                                    if(rotx[i] < 0)
                                        rotxvel[i] += multiplier * 8 * fabs(rotx[i]);
                                    if(rotx[i] > 0)
                                        rotxvel[i] -= multiplier * 4;
                                    if(rotx[i] < 0)
                                        rotxvel[i] += multiplier * 4;
                                    if(rotxvel[i] > 0)
                                        rotxvel[i] -= multiplier * 4;
                                ...
                                if(rotyvel[i] || roty[i]) {
                                    if(roty[i] > 0)
                                        rotyvel[i] -= multiplier * 8 * fabs(roty[i]);
                                    if(roty[i] < 0)
                                        rotyvel[i] += multiplier * 8 * fabs(roty[i]);
                                    if(roty[i] > 0)
                                        rotyvel[i] -= multiplier * 4;
                                    if(roty[i] < 0)
                                        rotyvel[i] += multiplier * 4;
                                    if(rotyvel[i] > 0)
                                        rotyvel[i] -= multiplier * 4;
                            ...
                            if(environment == snowyenvironment) {
                                if(type[i] == treeleavestype) {
                                    glRotatef((sin(windvar + position[i].x * .3) + .5) * 1.5 *
                                                  (sin(windvar * 2 + position[i].x * .3) + 1) / 2,
                                              1,
                                              0,
                                              0);
                                }
                                if(type[i] == treetrunktype) {
                                    glRotatef((sin(windvar + position[i].x * .3) + .5) * .5 *
                                                  (sin(windvar * 2 + position[i].x * .3) + 1) / 2,
                                              1,
                                              0,
                                              0);
                                ...
                                if(type[i] == bushtype) {
                                    glRotatef((sin(windvar + position[i].x * .3) + .5) * 4 *
                                                  (sin(windvar * 2 + position[i].x * .3) + 1) / 2,
                                              1,
                                              0,
                                              0);
                            ...
                            if(environment == grassyenvironment) {
                                if(type[i] == treeleavestype) {
                                    glRotatef((sin(windvar + position[i].x * .3) + .5) * 1.5 * .5 *
                                                  (sin(windvar * 2 + position[i].x * .3) + 1) / 2,
                                              1,
                                              0,
                                              0);
                                ...
                                if(type[i] == treetrunktype) {
                                    glRotatef((sin(windvar + position[i].x * .3) + .5) * .5 * .5 *
                                                  (sin(windvar * 2 + position[i].x * .3) + 1) / 2,
                                              1,
                                              0,
                                              0);
                                ...
                                if(type[i] == bushtype) {
                                    glRotatef((sin(windvar + position[i].x * .3) + .5) * 4 * .5 *
                                                  (sin(windvar * 2 + position[i].x * .3) + 1) / 2,
                                              1,
                                              0,
                                              0);
                            ...
                            if(type[i] != treetrunktype && type[i] != treeleavestype && type[i] != bushtype &&
                               type[i] != rocktype) {
                                glEnable(GL_CULL_FACE);
                                glAlphaFunc(GL_GREATER, 0.0001);
                                model[i].drawdifftex(boxtextureptr);
                                model[i].drawdecals(terrain.shadowtexture,
                                                    terrain.bloodtexture,
                                                    terrain.bloodtexture2,
                                                    terrain.breaktexture);
                            }
                            if(type[i] == rocktype) {
                                glEnable(GL_CULL_FACE);
                                glAlphaFunc(GL_GREATER, 0.0001);
                                glColor4f((1 - shadowed[i]) / 2 + light.ambient[0],
                                          (1 - shadowed[i]) / 2 + light.ambient[1],
                                          (1 - shadowed[i]) / 2 + light.ambient[2],
                                          distance);
    ...
    for(i = 0; i < numobjects; i++) {
        if(type[i] == treeleavestype || type[i] == bushtype) {
            moved = DoRotation(model[i].boundingspherecenter, 0, rotation[i], 0);
            if(frustum.SphereInFrustum(position[i].x + moved.x,
                                       position[i].y + moved.y,
                                       position[i].z + moved.z,
                                       model[i].boundingsphereradius)) {
                hidden = findDistancefastflat(&viewer, &position[i]) <= playerdist + 3;
...
{
    if((atype != treeleavestype && atype != bushtype) || config.video_foliage == 1) {
        scale[numobjects] = ascale;
        if(atype == treeleavestype)
            scale[numobjects] += fabs((float)(rand() % 100) / 900) * ascale;
        ...
        if(atype == rocktype && ascale > .5)
            friction[numobjects] = 1.5;
        if(atype == weirdtype)
            friction[numobjects] = 1.5;
        if(atype == spiketype)
            friction[numobjects] = .4;
        if(atype == treetrunktype)
            friction[numobjects] = .4;
        if(atype == treeleavestype)
        ...
        if(type[numobjects] == boxtype || type[numobjects] == cooltype || type[numobjects] == spiketype ||
           type[numobjects] == weirdtype || type[numobjects] == walltype || type[numobjects] == chimneytype ||
           type[numobjects] == tunneltype || type[numobjects] == platformtype) {
        ...
        if(atype == treetrunktype &&
           position[numobjects].y < terrain.getHeight(position[numobjects].x, position[numobjects].z) + 1) {
        ...
        if(atype != treeleavestype && atype != bushtype && atype != firetype)
            terrain.AddObject(where + DoRotation(model[numobjects].boundingspherecenter, 0, arotation, 0),
                              model[numobjects].boundingsphereradius,
                              numobjects);
...
void Objects::MakeObject(int atype, XYZ where, float arotation, float arotation2, float ascale)
{
    if((atype != treeleavestype && atype != bushtype) || config.video_foliage == 1) {
        ...
        if(friction[numobjects] == 1.5 && fabs(arotation2) > 5)
            friction[numobjects] = .5;
        ...
        if(onfire[i]) {
            ...
            while(flamedelay[i] < 0 && onfire[i]) {
                ...
                if(type[i] == bushtype || type[i] == firetype) {
                    ...
                    sprites.MakeSprite(flamesprite,
                                       spawnpoint,
                                       spawnpoint * 0,
                                       1,
                                       1,
                                       1,
                                       (.6 + (float)abs(rand() % 100) / 200 - .25) * 5 * scale[i],
                                       1);
                }
                if(type[i] == treeleavestype) {
        ...
        for(i = 0; i < numobjects; i++) {
            if(type[i] != treeleavestype && type[i] != treetrunktype && type[i] != bushtype && type[i] != firetype) {
                for(j = 0; j < model[i].vertexNum; j++) {
                    terrainpoint =
                        position[i] + DoRotation(model[i].vertex[j] + model[i].normals[j] * .1, 0, rotation[i], 0);
                        ...
                        if(terrain.patchobjectnum[patchx][patchz])
                            for(k = 0; k < terrain.patchobjectnum[patchx][patchz]; k++) {
                                ...
                                if(type[l] != treetrunktype /*&&l!=i*/) {
                                    ...
                                    if(model[l].LineCheck(&testpoint, &testpoint2, &col, &position[l], &rotation[l]) !=
                                       -1) {
