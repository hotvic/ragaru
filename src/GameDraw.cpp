...
/*********************> DrawGLScene() <*****/
long long Ragaru::Game::MD5_string (char *string) {
    char temp[256] = "";
    long long num = 90814;
...
int Ragaru::Game::DrawGLScene(void)
{
    ...
    if (!mainmenu)
    {
        ...
        if (config.video_blur && !loading) {
            if ((findLengthfast(&player[0].velocity) > 200) && config.video_blur_velocity && !cameramode) {
                drawmode = motionblurmode;
                motionbluramount = 200 / (findLengthfast(&player[0].velocity));
                changed = 1;
            }
            if (player[0].damage - player[0].superpermanentdamage > (player[0].damagetolerance - player[0].superpermanentdamage) * 1 / 2 && config.video_damage_effects && !cameramode) {
        ...
        if ((!changed && !slomo) || loading) {
            drawmode = normalmode;
            if (config.video_blur && config.video_always_blur) {
        ...
        if ((freeze || winfreeze) && config.video_blur && !mainmenu)drawmode = radialzoommode;
        ...
        if (!texcoordwidth) {
            ...
            texviewwidth = textureSize;
            if (texviewwidth > screenWidth)texviewwidth = screenWidth;
            texviewheight = textureSize;
            if (texviewheight > screenHeight)texviewheight = screenHeight;

            texcoordwidth = screenWidth / textureSize;
            texcoordheight = screenHeight / textureSize;
        ...
        */
        ...
        SetUpLight(&light, 0);
        glPushMatrix();
        if (environment == desertenvironment && config.video_detail == 2)glTexEnvf( GL_TEXTURE_FILTER_CONTROL_EXT, GL_TEXTURE_LOD_BIAS_EXT, blurness + .4 );
        if (environment == desertenvironment) {
        ...
        if (!console) {
            ...
            if (tutoriallevel == 1) {
                ...
                if (tutorialstage == 4) {
                    sprintf (string, "Try using the %s, %s, %s and %s keys to move around.", SDL_GetScancodeName(keys.forward), SDL_GetScancodeName(keys.left), SDL_GetScancodeName(keys.back), SDL_GetScancodeName(keys.right));
                    sprintf (string2, "All movement is relative to the camera.");
                    sprintf (string3, " ");
                }
                if (tutorialstage == 5) {
                    sprintf (string, "Please press %s to jump.", SDL_GetScancodeName(keys.jump));
                    sprintf (string2, "You can hold it longer to jump higher.");
                    sprintf (string3, " ");
                }
                if (tutorialstage == 6) {
                    sprintf (string, "You can press %s to crouch.", SDL_GetScancodeName(keys.crouch));
                    sprintf (string2, "You can jump higher from a crouching position.");
                    sprintf (string3, " ");
                }
                if (tutorialstage == 7) {
                    sprintf (string, "While running, you can press %s to roll.", SDL_GetScancodeName(keys.crouch));
                    sprintf (string2, " ");
                    sprintf (string3, " ");
                }
                if (tutorialstage == 8) {
                ...
                if (tutorialstage == 11) {
                    sprintf (string, "When you jump at a wall, you can hold %s again", SDL_GetScancodeName(keys.jump));
                ...
                if (tutorialstage == 12) {
                    sprintf (string, "While in the air, you can press '%s' to flip.", SDL_GetScancodeName(keys.crouch));
                ...
                if (tutorialstage == 15) {
                    sprintf(string, "Click with '%s' button to attack when you are near an enemy.", getButtonName(keys.attack));
                ...
                if (tutorialstage == 20) {
                    ...
                    sprintf(string3, "the jump key (%s) to attack.", SDL_GetScancodeName(keys.jump));
                }
                if (tutorialstage == 21) {
                ...
                if (tutorialstage == 24) {
                    ...
                    sprintf(string2, "and pressing jump (%s) or attack(mouse button).", SDL_GetScancodeName(keys.jump));
                ...
                if (tutorialstage == 28) {
                    ...
                    sprintf (string3, "crouch (%s) after attacking to escape from reversals.", SDL_GetScancodeName(keys.crouch));
                }
                if (tutorialstage == 29) {
                ...
                if (tutorialstage == 31) {
                    sprintf (string, "To reverse an attack, you must tap crouch (%s) during the", SDL_GetScancodeName(keys.crouch));
                ...
                if (tutorialstage == 40) {
                    ...
                    sprintf (string2, "while pressing %s to pick it up.", SDL_GetScancodeName(keys.throww));
                    sprintf (string3, "You can crouch and press the same key to drop it again.");
                }
                if (tutorialstage == 41) {
                    sprintf (string, "You can equip and unequip weapons using the %s key.", SDL_GetScancodeName(keys.draw));
                ...
                if (tutorialstage == 49) {
                    sprintf (string, "When facing an enemy, you can throw the knife with %s.", SDL_GetScancodeName(keys.throww));
...
//                  for(i=0;i<64;i++){
            ...
            if ((config.video_text || editorenabled) && config.game_debug && !mainmenu) {
                ...
                sprintf (string, "Difficulty: %d", config.game_difficulty);
        ...
        if ((((blackout && config.video_damage_effects) || (player[0].bloodloss > 0 && config.video_damage_effects && player[0].blooddimamount > 0) || player[0].dead) && !cameramode) || console) {
            glDisable(GL_DEPTH_TEST);                           // Disables Depth Testing
            glDisable(GL_CULL_FACE);
            glDisable(GL_LIGHTING);
            glDisable(GL_TEXTURE_2D);
        ...
        if (flashamount > 0 && config.video_damage_effects) {
        ...
        if (loading && !stealthloading && (!campaign || player[0].dead)) {
            ...
            loading = 2;
            //if(config.video_blur)drawmode=motionblurmode;
            drawmode = normalmode;
        }
    ...
    if (mainmenu) {
        ...
        if (mainmenu != oldmainmenu && oldmainmenu != 0)
        {
            ...
            if (mainmenu == 2)
            {
                LoadTexture("resume.png",  &Mainmenuitems[1], 0, 0);
                LoadTexture("endgame.png", &Mainmenuitems[3], 0, 0);
            }
        }
        ...
        if (mainmenu == 3 || mainmenu == 4 || mainmenu == 5 || mainmenu == 6 || mainmenu == 7 || mainmenu == 8 || mainmenu == 9 || mainmenu == 10 || mainmenu == 119 || mainmenu == 13 || mainmenu == 17) {
            ...
            //glScalef(.25,.25,.25);
            glBegin(GL_QUADS);
                glTexCoord2i(0, 0); glVertex3f(0, 0, 0.0f);
                glTexCoord2i(1, 0); glVertex3f(-1, 0, 0.0f);
                glTexCoord2f(1, 1); glVertex3f(-1, -1, 0.0f);
                glTexCoord2i(0, 1); glVertex3f(0, -1, 0.0f);
            glEnd();
            glPopMatrix();
            glPopMatrix();
            glPopMatrix();
            ...
            if (mainmenu == 3) {
                ...
                if (config.video_blood == 2)sprintf (menustring[2], "Blood: On, high detail (slower)");
                if (config.video_blood == 1)sprintf (menustring[2], "Blood: On, low detail");
                if (config.video_blood == 0)sprintf (menustring[2], "Blood: Off");
                startx[2] = 10 + 70;
                starty[2] = 370;
                ...
                if (config.game_difficulty == 2)sprintf (menustring[3], "Difficulty: Insane");
                if (config.game_difficulty == 1)sprintf (menustring[3], "Difficulty: Difficult");
                if (config.game_difficulty == 0)sprintf (menustring[3], "Difficulty: Easier");
                startx[3] = 10 + 20 - 1000;
                starty[3] = 335 - 1000;
                ...
                if (config.video_blur == 1)sprintf (menustring[4], "Blur Effects: Enabled (less compatible)");
                if (config.video_blur == 0)sprintf (menustring[4], "Blur Effects: Disabled (more compatible)");
                startx[4] = 10;
                starty[4] = 335;
                ...
                if (config.video_decals == 1)sprintf (menustring[5], "Decals: Enabled (slower)");
                if (config.video_decals == 0)sprintf (menustring[5], "Decals: Disabled");
                startx[5] = 10 + 60;
                starty[5] = 300;
                ...
                if (config.sound_music == 1)sprintf (menustring[6], "Music: Enabled");
                if (config.sound_music == 0)sprintf (menustring[6], "Music: Disabled");
                startx[6] = 10 + 70;
                starty[6] = 265;
                ...
                if (config.input_mouse_invert == 1)sprintf (menustring[9], "Invert mouse: Yes");
                if (config.input_mouse_invert == 0)sprintf (menustring[9], "Invert mouse: No");
                startx[9] = 10;
                starty[9] = 230;
                ...
                sprintf (menustring[10], "Mouse Speed: %d", config.input_mouse_speed);
                startx[10] = 20;
                starty[10] = 195;
                ...
                sprintf (menustring[11], "Volume: %d%%", config.sound_volume);
                startx[11] = 10 + 60;
                starty[11] = 155;
                ...
                if (newdetail == config.video_detail && newscreenheight == (int)screenHeight && newscreenwidth == (int)screenWidth)sprintf (menustring[8], "Back");
            ...
            if (mainmenu == 4) {
                nummenuitems = 10;
                if (keyselect != 0)sprintf (menustring[0], "Forwards: %s", SDL_GetScancodeName(keys.forward));
                ...
                if (keyselect != 1)sprintf (menustring[1], "Back: %s", SDL_GetScancodeName(keys.back));
                ...
                if (keyselect != 2)sprintf (menustring[2], "Left: %s", SDL_GetScancodeName(keys.left));
                ...
                if (keyselect != 3)sprintf (menustring[3], "Right: %s", SDL_GetScancodeName(keys.right));
                ...
                if (keyselect != 4)sprintf (menustring[4], "Crouch: %s", SDL_GetScancodeName(keys.crouch));
                ...
                if (keyselect != 5)sprintf (menustring[5], "Jump: %s", SDL_GetScancodeName(keys.jump));
                ...
                if (keyselect != 6)sprintf (menustring[6], "Draw: %s", SDL_GetScancodeName(keys.draw));
                ...
                if (keyselect != 7)sprintf (menustring[7], "Throw: %s", SDL_GetScancodeName(keys.throww));
                ...
                if (keyselect != 8)sprintf (menustring[8], "Attack: %s", getButtonName(keys.attack));
        ...
        if (mainmenu == 1 || mainmenu == 2)
        {
            for (i = 1; i < 4; i++)
            {
                if (isMouseHoverRect(startx[i], endx[i], starty[i], endy[i]))
                {
                    selected = i;
                }
            }
        ...
        if (mainmenu == 1 || mainmenu == 2) {
            ...
            //glScalef(.25,.25,.25);
            glBegin(GL_QUADS);
                glTexCoord2i(0, 0); glVertex3f(-1,  1, 0.0f);
                glTexCoord2i(1, 0); glVertex3f( 1,  1, 0.0f);
                glTexCoord2i(1, 1); glVertex3f( 1, -1, 0.0f);
                glTexCoord2i(0, 1); glVertex3f(-1, -1, 0.0f);
            glEnd();
            glPopMatrix();
            glPopMatrix();
            glPopMatrix();
            ...
            for (j = 0; j < nummenuitems; j++)
            {
                if (j <= 3 || (mainmenu != 1 && mainmenu != 2))
                {
                    ...
                    if (mainmenu == 1 || mainmenu == 2)
                    {
                        ...
                        glPushMatrix();
                        glBegin(GL_QUADS);
                            glTexCoord2i(0, 0); glVertex3f(startx[j] + movex[j] * transition, endy[j] + movey[j] * transition,   0.0f);
                            glTexCoord2i(0, 1); glVertex3f(startx[j] + movex[j] * transition, starty[j] + movey[j] * transition, 0.0f);
                            glTexCoord2i(1, 1); glVertex3f(endx[j] + movex[j] * transition,   starty[j] + movey[j] * transition, 0.0f);
                            glTexCoord2i(1, 0); glVertex3f(endx[j] + movex[j] * transition,   endy[j] + movey[j] * transition,   0.0f);
                        glEnd();
                        glPopMatrix();
                        glEnable(GL_BLEND);
                        //glDisable(GL_ALPHA_TEST);
