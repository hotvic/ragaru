...
int Game::DrawGLScene(void)
{
    ...
    if(!mainmenu) {
        if(editorenabled) {
            ...
            for(i = 0; i < numboundaries; i++) {
                ...
                boundary[i].z = 1;
                boundary[i] =
                    mapcenter + DoRotation(boundary[i] * mapradius, 0, i * (360 / ((float)(numboundaries))), 0);
            }
        }
        ...
        if(ismotionblur && !loading) {
            ...
            if(player[0].damage - player[0].superpermanentdamage >
                   (player[0].damagetolerance - player[0].superpermanentdamage) * 1 / 2 &&
               damageeffects && !cameramode) {
        ...
        /*if(environment==desertenvironment)glTexEnvf( GL_TEXTURE_FILTER_CONTROL_EXT, GL_TEXTURE_LOD_BIAS_EXT,
        (float)(abs(rand()%100))/50 );
        ...
        */
        ...
        for(k = 0; k < numplayers; k++) {
            if(!player[k].skeleton.free && player[k].playerdetail && player[k].howactive < typesleeping)
                if(frustum.SphereInFrustum(player[k].coords.x,
                                           player[k].coords.y + player[k].scale * 3,
                                           player[k].coords.z,
                                           player[k].scale * 7) &&
                   player[k].occluded < 25)
                    for(i = 0; i < player[k].skeleton.num_joints; i++) {
                        if(player[k].skeleton.joints[i].label == leftknee ||
                           player[k].skeleton.joints[i].label == rightknee ||
                           player[k].skeleton.joints[i].label == groin) {
                            point = DoRotation(player[k].skeleton.joints[i].position, 0, player[k].rotation, 0) *
                                        player[k].scale +
                                    player[k].coords;
                            size = .4f;
                            opacity =
                                .4 - player[k].skeleton.joints[i].position.y * player[k].scale / 5 -
                                (player[k].coords.y - terrain.getHeight(player[k].coords.x, player[k].coords.z)) / 10;
                            if(k != 0 && tutoriallevel == 1) {
                                opacity =
                                    .2 + .2 * sin(smoketex * 6 + i) -
                                    player[k].skeleton.joints[i].position.y * player[k].scale / 5 -
                            ...
                            if(terrain.patchobjectnum[player[k].whichpatchx][player[k].whichpatchz])
                                for(l = 0; l < terrain.patchobjectnum[player[k].whichpatchx][player[k].whichpatchz];
                                    l++) {
                                    j = terrain.patchobjects[player[k].whichpatchx][player[k].whichpatchz][l];
                                    if(objects.position[j].y < player[k].coords.y || objects.type[j] == tunneltype ||
                                       objects.type[j] == weirdtype) {
                                        point = DoRotation(
                                            DoRotation(
                                                player[k].skeleton.joints[i].position, 0, player[k].rotation, 0) *
                                                    player[k].scale +
                                                player[k].coords - objects.position[j],
                                            0,
                                            -objects.rotation[j],
                                            0);
                                        size = .4f;
            ...
            if((player[k].skeleton.free || player[k].howactive >= typesleeping) && player[k].playerdetail)
                if(frustum.SphereInFrustum(
                       player[k].coords.x, player[k].coords.y, player[k].coords.z, player[k].scale * 5) &&
                   player[k].occluded < 25)
                    for(i = 0; i < player[k].skeleton.num_joints; i++) {
                        if(player[k].skeleton.joints[i].label == leftknee ||
                           player[k].skeleton.joints[i].label == rightknee ||
                           player[k].skeleton.joints[i].label == groin ||
                           player[k].skeleton.joints[i].label == leftelbow ||
                           player[k].skeleton.joints[i].label == rightelbow ||
                           player[k].skeleton.joints[i].label == neck) {
                            ...
                            opacity =
                                .4 - player[k].skeleton.joints[i].position.y * player[k].scale / 5 -
                                (player[k].coords.y - terrain.getHeight(player[k].coords.x, player[k].coords.z)) / 5;
                            if(k != 0 && tutoriallevel == 1) {
                                opacity =
                                    .2 + .2 * sin(smoketex * 6 + i) -
                                    player[k].skeleton.joints[i].position.y * player[k].scale / 5 -
                            ...
                            if(terrain.patchobjectnum[player[k].whichpatchx][player[k].whichpatchz])
                                for(l = 0; l < terrain.patchobjectnum[player[k].whichpatchx][player[k].whichpatchz];
                                    ...
                                    if(objects.position[j].y < player[k].coords.y || objects.type[j] == tunneltype ||
                                       objects.type[j] == weirdtype) {
                                        if(player[k].skeleton.free)
                                            point = DoRotation(player[k].skeleton.joints[i].position * player[k].scale +
                                                                   player[k].coords - objects.position[j],
                                                               0,
                                                               -objects.rotation[j],
                                                               0);
        ...
        if(!cellophane) {
            ...
            for(k = 0; k < numplayers; k++) {
                if(k == 0 || tutoriallevel != 1) {
                    ...
                    if(distance >= .5) {
                        checkpoint =
                            DoRotation(player[k].skeleton.joints[abs(rand() % player[k].skeleton.num_joints)].position,
                                       0,
                                       player[k].rotation,
                                       0) *
                                player[k].scale +
        ...
        if(!cameramode && musictype == stream_music2)
        ...
        glPushMatrix();
        if(frustum.SphereInFrustum(realhawkcoords.x + hawk.boundingspherecenter.x,
                                   realhawkcoords.y + hawk.boundingspherecenter.y,
                                   realhawkcoords.z + hawk.boundingspherecenter.z,
                                   2)) {
            glAlphaFunc(GL_GREATER, 0.0001f);
            glDepthMask(1);
        ...
        if(editorenabled) {
            ...
            if(numpathpoints > 1) {
                ...
                for(k = 0; k < numpathpoints; k++) {
                    if(numpathpointconnect[k]) {
                        for(i = 0; i < numpathpointconnect[k]; i++) {
                            ...
                            glVertex3f(pathpoint[pathpointconnect[k][i]].x,
                                       pathpoint[pathpointconnect[k][i]].y + .5,
                                       pathpoint[pathpointconnect[k][i]].z);
                            glEnd();
                        }
                    }
                }
                ...
                glPointSize(4);
                glBegin(GL_POINTS);
                glVertex3f(pathpoint[pathpointselected].x,
                           pathpoint[pathpointselected].y + .5,
                           pathpoint[pathpointselected].z);
                glEnd();
            }
        }
        ...
        if(!console) {
            ...
            if(!tutoriallevel)
                if(bonus > 0 && bonustime < 1 && !winfreeze && indialogue == -1 /*bonustime<4*/) {
                    if(bonus == tracheotomy)
                        sprintf(string, "Tracheotomy!");
                    else if(bonus == backstab)
                        sprintf(string, "Backstabber!");
                    else if(bonus == spinecrusher)
                        sprintf(string, "Spinecrusher!");
                    else if(bonus == ninja)
                        sprintf(string, "Ninja Bonus!");
                    else if(bonus == style)
                        sprintf(string, "Style Bonus!");
                    else if(bonus == cannon)
                        sprintf(string, "Leg Cannon!");
                    else if(bonus == aimbonus)
                        sprintf(string, "Nice Aim!");
                    else if(bonus == deepimpact)
                        sprintf(string, "Heavy Impact!");
                    else if(bonus == touchofdeath)
                        sprintf(string, "Touch of Death!");
                    else if(bonus == swordreversebonus)
                        sprintf(string, "Sword Disarm!");
                    else if(bonus == staffreversebonus)
                        sprintf(string, "Staff Disarm!");
                    else if(bonus == reverseko)
                        sprintf(string, "Reversal KO!");
                    else if(bonus == solidhit)
                        sprintf(string, "Solid Hit!");
                    else if(bonus == twoxcombo)
                        sprintf(string, "2X Combo!");
                    else if(bonus == threexcombo)
                        sprintf(string, "3X Combo!");
                    else if(bonus == fourxcombo)
                        sprintf(string, "4X COMBO!");
                    else if(bonus == megacombo)
                        sprintf(string, "MEGA COMBO!");
                    else if(bonus == Reversal)
                        sprintf(string, "Reversal!");
                    else if(bonus == Stabbonus)
                        sprintf(string, "Punctured!");
                    else if(bonus == Slicebonus)
                        sprintf(string, "Sliced!");
                    else if(bonus == Bullseyebonus)
                        sprintf(string, "Bullseye!");
                    else if(bonus == Slashbonus)
                        sprintf(string, "Slashed!");
                    else if(bonus == Wolfbonus)
                        sprintf(string, "WOLF SLAYER!");
                    else if(bonus == FinishedBonus)
                        sprintf(string, "SLAIN!");
                    else if(bonus == TackleBonus)
                        sprintf(string, "Tackle!");
                    else if(bonus == AboveBonus)
                        sprintf(string, "Death from Above!");
                    else
                        sprintf(string, "Excellent!");

                    glColor4f(0, 0, 0, 1 - bonustime);
                    text.glPrintOutline(
                        1024 / 2 - 10 * strlen(string) - 4, 768 / 16 - 4 + 768 * 4 / 5, string, 1, 2.5, 1024, 768);
                    ...
                    glColor4f(0, 0, 0, 1 - bonustime);
                    text.glPrintOutline(1024 / 2 - 10 * strlen(string) - 4,
                                        768 / 16 - 4 - 20 + 768 * 4 / 5,
                                        string,
                                        1,
                                        2.5 * .8,
                    ...
                    glColor4f(1, 0, 0, 1 - bonustime);
                    text.glPrint(
                        1024 / 2 - 10 * strlen(string), 768 / 16 - 20 + 768 * 4 / 5, string, 1, 2 * .8, 1024, 768);
                    glColor4f(.5, .5, .5, 1);
                }
            ...
            if(tutoriallevel == 1) {
                ...
                if(tutorialstage == 4) {
                    sprintf(string,
                            "Try using the %s, %s, %s and %s keys to move around.",
                            SDL_GetScancodeName(forwardkey),
                            SDL_GetScancodeName(leftkey),
                            SDL_GetScancodeName(backkey),
                            SDL_GetScancodeName(rightkey));
                    sprintf(string2, "All movement is relative to the camera.");
                    sprintf(string3, " ");
                }
                if(tutorialstage == 5) {
                    sprintf(string, "Please press %s to jump.", SDL_GetScancodeName(jumpkey));
                    sprintf(string2, "You can hold it longer to jump higher.");
                    sprintf(string3, " ");
                }
                if(tutorialstage == 6) {
                    sprintf(string, "You can press %s to crouch.", SDL_GetScancodeName(crouchkey));
                    sprintf(string2, "You can jump higher from a crouching position.");
                    sprintf(string3, " ");
                }
                if(tutorialstage == 7) {
                    sprintf(string, "While running, you can press %s to roll.", SDL_GetScancodeName(crouchkey));
                    sprintf(string2, " ");
                    sprintf(string3, " ");
                }
                if(tutorialstage == 8) {
                ...
                if(tutorialstage == 11) {
                    sprintf(string, "When you jump at a wall, you can hold %s again", SDL_GetScancodeName(jumpkey));
                ...
                if(tutorialstage == 12) {
                    sprintf(string, "While in the air, you can press %s to flip.", SDL_GetScancodeName(crouchkey));
                ...
                if(tutorialstage == 20) {
                    ...
                    sprintf(string3, "the jump key (%s) to attack.", SDL_GetScancodeName(jumpkey));
                }
                if(tutorialstage == 21) {
                ...
                if(tutorialstage == 24) {
                    ...
                    sprintf(string2, "and pressing jump (%s) or attack(mouse button).", SDL_GetScancodeName(jumpkey));
                ...
                if(tutorialstage == 28) {
                    ...
                    sprintf(string3,
                            "crouch (%s) after attacking to escape from reversals.",
                            SDL_GetScancodeName(crouchkey));
                }
                if(tutorialstage == 29) {
                ...
                if(tutorialstage == 31) {
                    sprintf(string,
                            "To reverse an attack, you must tap crouch (%s) during the",
                            SDL_GetScancodeName(crouchkey));
                ...
                if(tutorialstage == 37) {
                    sprintf(string,
                            "Now spar with the enemy for %d more seconds.",
                            (int)(tutorialmaxtime - tutorialstagetime));
                ...
                if(tutorialstage == 40) {
                    ...
                    sprintf(string2, "while pressing %s to pick it up.", SDL_GetScancodeName(throwkey));
                    sprintf(string3, "You can crouch and press the same key to drop it again.");
                }
                if(tutorialstage == 41) {
                    sprintf(
                        string, "You can equip and unequip weapons using the %s key.", SDL_GetScancodeName(drawkey));
                ...
                if(tutorialstage == 49) {
                    sprintf(string,
                            "When facing an enemy, you can throw the knife with %s.",
                            SDL_GetScancodeName(throwkey));
                ...
                text.glPrintOutline(screenwidth / 2 - 7.6 * strlen(string) * screenwidth / 1024 - 4,
                                    screenheight / 16 - 4 + screenheight * 4 / 5,
                                    string,
                                    1,
                                    1.5 * 1.25 * screenwidth / 1024,
                                    screenwidth,
                                    screenheight);
                text.glPrintOutline(screenwidth / 2 - 7.6 * strlen(string2) * screenwidth / 1024 - 4,
                                    screenheight / 16 - 4 + screenheight * 4 / 5 - 20 * screenwidth / 1024,
                                    string2,
                                    1,
                                    1.5 * 1.25 * screenwidth / 1024,
                                    screenwidth,
                                    screenheight);
                text.glPrintOutline(screenwidth / 2 - 7.6 * strlen(string3) * screenwidth / 1024 - 4,
                                    screenheight / 16 - 4 + screenheight * 4 / 5 - 40 * screenwidth / 1024,
                                    string3,
                                    1,
                                    1.5 * 1.25 * screenwidth / 1024,
                                    screenwidth,
                ...
                text.glPrint(screenwidth / 2 - 7.6 * strlen(string) * screenwidth / 1024,
                             screenheight / 16 + screenheight * 4 / 5,
                             string,
                             1,
                             1.5 * screenwidth / 1024,
                             screenwidth,
                             screenheight);
                text.glPrint(screenwidth / 2 - 7.6 * strlen(string2) * screenwidth / 1024,
                             screenheight / 16 + screenheight * 4 / 5 - 20 * screenwidth / 1024,
                             string2,
                             1,
                             1.5 * screenwidth / 1024,
                             screenwidth,
                             screenheight);
                text.glPrint(screenwidth / 2 - 7.6 * strlen(string3) * screenwidth / 1024,
                             screenheight / 16 + screenheight * 4 / 5 - 40 * screenwidth / 1024,
                             string3,
                             1,
                             1.5 * screenwidth / 1024,
                             screenwidth,
                ...
                glColor4f(0, 0, 0, 1);
                text.glPrintOutline(screenwidth / 2 - 7.6 * strlen(string) * screenwidth / 1024 * .8 - 4,
                                    0 - 4 + screenheight * 1 / 10,
                                    string,
                                    1,
                                    1.5 * 1.25 * screenwidth / 1024 * .8,
                                    screenwidth,
                                    screenheight);
                text.glPrintOutline(screenwidth / 2 - 7.6 * strlen(string2) * screenwidth / 1024 * .8 - 4,
                                    0 - 4 + screenheight * 1 / 10 - 20 * .8 * screenwidth / 1024,
                                    string2,
                                    1,
                                    1.5 * 1.25 * screenwidth / 1024 * .8,
                                    screenwidth,
                                    screenheight);
                text.glPrintOutline(screenwidth / 2 - 7.6 * strlen(string3) * screenwidth / 1024 * .8 - 4,
                                    0 - 4 + screenheight * 1 / 10 - 40 * .8 * screenwidth / 1024,
                                    string3,
                                    1,
                                    1.5 * 1.25 * screenwidth / 1024 * .8,
                                    screenwidth,
                ...
                text.glPrint(screenwidth / 2 - 7.6 * strlen(string) * screenwidth / 1024 * .8,
                             0 + screenheight * 1 / 10,
                             string,
                             1,
                             1.5 * screenwidth / 1024 * .8,
                             screenwidth,
                             screenheight);
                text.glPrint(screenwidth / 2 - 7.6 * strlen(string2) * screenwidth / 1024 * .8,
                             0 + screenheight * 1 / 10 - 20 * .8 * screenwidth / 1024,
                             string2,
                             1,
                             1.5 * screenwidth / 1024 * .8,
                             screenwidth,
                             screenheight);
                text.glPrint(screenwidth / 2 - 7.6 * strlen(string3) * screenwidth / 1024 * .8,
                             0 + screenheight * 1 / 10 - 40 * .8 * screenwidth / 1024,
                             string3,
                             1,
                             1.5 * screenwidth / 1024 * .8,
                             screenwidth,
                ...
                for(i = 0; i < numhotspots; i++) {
                    ...
                    if(closestdist == -1 || distance < closestdist) {
                        if(findDistancefast(&player[0].coords, &hotspot[i]) < hotspotsize[i] &&
                           ((hotspottype[i] <= 10 && hotspottype[i] >= 0) ||
                            (hotspottype[i] <= 40 && hotspottype[i] >= 20))) {
                    ...
                    if(hotspottype[closest] <= 10) {
                        ...
                        while(!done) {
                            if(string[i] == '\n' || string[i] > 'z' || string[i] < ' ' || string[i] == '\0') {
                                glColor4f(0, 0, 0, tutorialopac);
                                text.glPrintOutline(screenwidth / 2 - 7.6 * (i - lastline) * screenwidth / 1024 - 4,
                                                    screenheight / 16 - 4 + screenheight * 4 / 5 -
                                                        20 * screenwidth / 1024 * line,
                                                    string,
                                                    1,
                                                    1.5 * 1.25 * screenwidth / 1024,
                                                    screenwidth,
                                                    ...
                                                    i);
                                glColor4f(1, 1, 1, tutorialopac);
                                text.glPrint(screenwidth / 2 - 7.6 * (i - lastline) * screenwidth / 1024,
                                             screenheight / 16 + screenheight * 4 / 5 - 20 * screenwidth / 1024 * line,
                                             string,
                                             1,
                                             1.5 * screenwidth / 1024,
                                             screenwidth,
            ...
            if(indialogue != -1 && !mainmenu) {
                ...
                glColor4f(dialogueboxcolor[whichdialogue][indialogue][0],
                          dialogueboxcolor[whichdialogue][indialogue][1],
                          dialogueboxcolor[whichdialogue][indialogue][2],
                          0.7);
                glBegin(GL_QUADS);
                glVertex3f(0, 0, 0.0f);
                glVertex3f(1, 0, 0.0f);
                ...
                for(i = 0; i < (int)strlen(dialoguename[whichdialogue][indialogue]); i++) {
                    ...
                    goodchar = 1;
                    if(dialoguename[whichdialogue][indialogue][i] == '#' ||
                       dialoguename[whichdialogue][indialogue][i] == '\0')
                        goodchar = 0;
                ...
                if(dialogueboxcolor[whichdialogue][indialogue][0] + dialogueboxcolor[whichdialogue][indialogue][1] +
                       dialogueboxcolor[whichdialogue][indialogue][2] <
                   1.5) {
                    glColor4f(0, 0, 0, tutorialopac);
                    text.glPrintOutline(startx - 2 * 7.6 * strlen(string) * screenwidth / 1024 - 4,
                                        starty - 4,
                                        string,
                                        1,
                                        1.5 * 1.25 * screenwidth / 1024,
                    ...
                    text.glPrint(startx - 2 * 7.6 * strlen(string) * screenwidth / 1024,
                                 starty,
                                 string,
                                 1,
                                 1.5 * screenwidth / 1024,
                ...
                while(!done) {
                    if(string[i] == '\n' || string[i] > 'z' || string[i] < ' ' || string[i] == '\0') {
                        if(dialogueboxcolor[whichdialogue][indialogue][0] +
                               dialogueboxcolor[whichdialogue][indialogue][1] +
                               dialogueboxcolor[whichdialogue][indialogue][2] <
                           1.5) {
                            glColor4f(0, 0, 0, tutorialopac);
                            text.glPrintOutline(startx /*-7.6*(i-lastline)*screenwidth/1024*/ - 4,
                                                starty - 4 - 20 * screenwidth / 1024 * line,
                                                string,
                                                1,
                                                1.5 * 1.25 * screenwidth / 1024,
                                                screenwidth,
                                                ...
                                                i);
                            glColor4f(1, 1, 1, tutorialopac);
                            text.glPrint(startx /*-7.6*(i-lastline)*screenwidth/1024*/,
                                         starty - 20 * screenwidth / 1024 * line,
                                         string,
                                         1,
                                         1.5 * screenwidth / 1024,
                                         screenwidth,
            ...
            if(!tutoriallevel && !winfreeze && indialogue == -1 && !mainmenu) {
                if(campaign) {
                    if(!scoreadded)
                        sprintf(string,
                                "Score: %d",
                                (int)accountcampaignscore[accountactive] + (int)bonustotal); //(int)bonustotal);
                    if(scoreadded)
                        sprintf(string, "Score: %d", (int)accountcampaignscore[accountactive]); //(int)bonustotal);
                }
                if(!campaign)
            ...
            if((texttoggle || editorenabled) && debugmode && !mainmenu) {
                ...
                if(editorenabled) {
                    ...
                    if(editorrotation >= 0)
                        sprintf(string, "Object rotation: %f", editorrotation);
                    else
                        sprintf(string, "Object rotation: rand");
                    text.glPrint(10, 90, string, 0, .8, 1024, 768);
                    if(editorrotation2 >= 0)
                        sprintf(string, "Object rotation2: %f", editorrotation2);
                    else
                        sprintf(string, "Object rotation2: rand");
                    text.glPrint(10, 105, string, 0, .8, 1024, 768);
                    sprintf(string, "Object type: %d", editortype);
                    text.glPrint(10, 120, string, 0, .8, 1024, 768);
                    if(editortype == boxtype)
                        sprintf(string, "(box)");
                    if(editortype == treetrunktype)
                        sprintf(string, "(tree)");
                    if(editortype == walltype)
                        sprintf(string, "(wall)");
                    if(editortype == weirdtype)
                        sprintf(string, "(weird)");
                    if(editortype == spiketype)
                        sprintf(string, "(spike)");
                    if(editortype == rocktype)
                        sprintf(string, "(rock)");
                    if(editortype == bushtype)
                        sprintf(string, "(bush)");
                    if(editortype == tunneltype)
                        sprintf(string, "(tunnel)");
                    if(editortype == chimneytype)
                        sprintf(string, "(chimney)");
                    if(editortype == platformtype)
                        sprintf(string, "(platform)");
                    if(editortype == cooltype)
                        sprintf(string, "(cool)");
        ...
        if((((blackout && damageeffects) ||
             (player[0].bloodloss > 0 && damageeffects && player[0].blooddimamount > 0) || player[0].dead) &&
            !cameramode) ||
           console) {
            glDisable(GL_DEPTH_TEST); // Disables Depth Testing
            glDisable(GL_CULL_FACE);
            ...
            if(!player[0].dead) {
                if((player[0].bloodloss / player[0].damagetolerance * (sin(woozy) / 4 + .5)) * .3 < .3) {
                    glColor4f(0,
                              0,
                              0,
                              player[0].blooddimamount * player[0].bloodloss / player[0].damagetolerance *
                                  (sin(woozy) / 4 + .5) * .3);
                    blackout = player[0].blooddimamount * player[0].bloodloss / player[0].damagetolerance *
                               (sin(woozy) / 4 + .5) * .3;
                } else {
        ...
        if(!console) {
            ...
            if(chatting) {
                ...
                if(displayblink) {
                    sprintf(string, "_");
                    text.glPrint(30 + (float)(displayselected) * 10,
                                 30 + (screenheight - 330),
                                 string,
                                 0,
                                 1,
                ...
                if((i != 0 || chatting) && displaytime[i] < 4)
                    for(j = 0; j < displaychars[i]; j++) {
                        ...
                        if(j < displaychars[i]) {
                            ...
                            text.glPrint(30 + j * 10,
                                         30 + i * 20 + (screenheight - 330),
                                         string,
                                         0,
                                         1,
                                         screenwidth,
        ...
        if(minimap && indialogue == -1) {
            ...
            for(i = 0; i < objects.numobjects; i++) {
                if(objects.type[i] == treetrunktype) {
                    ...
                    if(distcheck < mapviewdist) {
                        ...
                        glPushMatrix();
                        glTranslatef(objects.position[i].x / terrain.scale / 256 * -2 + 1,
                                     objects.position[i].z / terrain.scale / 256 * 2 - 1,
                                     0);
            ...
            if(editorenabled) {
                ...
                for(i = 0; i < numboundaries; i++) {
                    ...
                    glPushMatrix();
                    glTranslatef(
                        boundary[i].x / terrain.scale / 256 * -2 + 1, boundary[i].z / terrain.scale / 256 * 2 - 1, 0);
            ...
            for(i = 0; i < numplayers; i++) {
                ...
                if(distcheck < mapviewdist) {
                    ...
                    if(i == 0)
                        ...
                        glColor4f(1, 1, 0, 1);
                    glTranslatef(player[i].coords.x / terrain.scale / 256 * -2 + 1,
                                 player[i].coords.z / terrain.scale / 256 * 2 - 1,
                                 0);
        ...
        if(loading && !stealthloading && (!campaign || player[0].dead)) {
            ...
            glDepthMask(0);
            glMatrixMode(GL_PROJECTION);						// Select The Projection Matrix
            glPushMatrix();										// Store The
            Projection
            Matrix
            glLoadIdentity();									// Reset The Projection
            Matrix
            glOrtho(0,screenwidth,0,screenheight,-100,100);						// Set Up An
            Ortho
            Screen
            glMatrixMode(GL_MODELVIEW);							// Select The Modelview Matrix
            glPushMatrix();										// Store The
        ...
        if(winfreeze && !campaign) {
            ...
            for(i = 0; i < numawards; i++) {
                if(i < 6) {
                    if(awards[i] == awardklutz)
                        sprintf(string, "Suicidal");
                    if(awards[i] == awardflawless)
                        sprintf(string, "Flawless!");
                    if(awards[i] == awardalldead)
                        sprintf(string, "Take no prisoners");
                    if(awards[i] == awardnodead)
                        sprintf(string, "Merciful");
                    if(awards[i] == awardstealth)
                        sprintf(string, "One with the shadows!");
                    if(awards[i] == awardswordsman)
                        sprintf(string, "Swordsman");
                    if(awards[i] == awardkungfu)
                        sprintf(string, "Unarmed!");
                    if(awards[i] == awardknifefighter)
                        sprintf(string, "Knife fighter");
                    if(awards[i] == awardcoward)
                        sprintf(string, "Coward");
                    if(awards[i] == awardevasion)
                        sprintf(string, "Escape artist");
                    if(awards[i] == awardacrobat)
                        sprintf(string, "Gymnast");
                    if(awards[i] == awardlongrange)
                        sprintf(string, "Blade slinger");
                    if(awards[i] == awardbrutal)
                        sprintf(string, "Brutal");
                    if(awards[i] == awardhyper)
                        sprintf(string, "Hyper");
                    if(awards[i] == awardaikido)
                        sprintf(string, "Aikido master!");
                    if(awards[i] == awardrambo)
                        sprintf(string, "Rambo");
                    if(awards[i] == awardfast)
                        sprintf(string, "Fast");
                    if(awards[i] == awardrealfast)
                        sprintf(string, "Real fast");
                    if(awards[i] == awarddamnfast)
                        sprintf(string, "Damn fast");
                    if(awards[i] == awardstrategy)
                        sprintf(string, "Divide and conquer");
                    if(awards[i] == awardbojutsu)
                        sprintf(string, "Bojutsu");
        ...
        if(drawmode != normalmode) {
            ...
            if(drawmode == doublevisionmode) {
                ...
                crosseyedness = abs(player[0].damage - player[0].superpermanentdamage -
                                    (player[0].damagetolerance - player[0].superpermanentdamage) * 1 / 2) /
                                30;
                if(crosseyedness > 1)
                    crosseyedness = 1;
    ...
    if(freeze || winfreeze || (mainmenu && gameon) || (!gameon && gamestarted) || (mainmenu && gameon) ||
    ...
    if(mainmenu) {
            ...
            if(mainmenu == 5) {
                ...
                if(whichlevelstart < 0) {
                    ...
                    if(campaignchoicenum == 0) {
                        ...
                        if(accountcampaignfasttime[accountactive] == 0 ||
                           accountcampaigntime[accountactive] < accountcampaignfasttime[accountactive])
                            accountcampaignfasttime[accountactive] = accountcampaigntime[accountactive];
                    }
        ...
        if(mainmenu == 3 || mainmenu == 4 || mainmenu == 5 || mainmenu == 6 || mainmenu == 7 || mainmenu == 8 ||
           mainmenu == 9 || mainmenu == 10 || mainmenu == 119 || mainmenu == 13 || mainmenu == 17) {
            glClear(GL_DEPTH_BUFFER_BIT);
            glEnable(GL_ALPHA_TEST);
            ...
            if(mainmenu == 3) {
                nummenuitems = 12;
                if((float)newscreenwidth > (float)newscreenheight * 1.61 ||
                   (float)newscreenwidth < (float)newscreenheight * 1.59)
                ...
                if(newdetail == 2)
                    sprintf(menustring[1], "Detail: High");
                else if(newdetail == 1)
                    sprintf(menustring[1], "Detail: Medium");
                else
                    sprintf(menustring[1], "Detail: Low");
                startx[1] = 10 + 60;
                starty[1] = 405;
                ...
                if(bloodtoggle == 2)
                    sprintf(menustring[2], "Blood: On, high detail (slower)");
                if(bloodtoggle == 1)
                    sprintf(menustring[2], "Blood: On, low detail");
                if(bloodtoggle == 0)
                    sprintf(menustring[2], "Blood: Off");
                startx[2] = 10 + 70;
                starty[2] = 370;
                ...
                if(difficulty == 2)
                    sprintf(menustring[3], "Difficulty: Insane");
                if(difficulty == 1)
                    sprintf(menustring[3], "Difficulty: Difficult");
                if(difficulty == 0)
                    sprintf(menustring[3], "Difficulty: Easier");
                startx[3] = 10 + 20 - 1000;
                starty[3] = 335 - 1000;
                ...
                if(ismotionblur == 1)
                    sprintf(menustring[4], "Blur Effects: Enabled (less compatible)");
                if(ismotionblur == 0)
                    sprintf(menustring[4], "Blur Effects: Disabled (more compatible)");
                startx[4] = 10;
                starty[4] = 335;
                ...
                if(decals == 1)
                    sprintf(menustring[5], "Decals: Enabled (slower)");
                if(decals == 0)
                    sprintf(menustring[5], "Decals: Disabled");
                startx[5] = 10 + 60;
                starty[5] = 300;
                ...
                if(musictoggle == 1)
                    sprintf(menustring[6], "Music: Enabled");
                if(musictoggle == 0)
                    sprintf(menustring[6], "Music: Disabled");
                startx[6] = 10 + 70;
                starty[6] = 265;
                ...
                if(invertmouse == 1)
                    sprintf(menustring[9], "Invert mouse: Yes");
                if(invertmouse == 0)
                    sprintf(menustring[9], "Invert mouse: No");
                startx[9] = 10;
                starty[9] = 230;
            ...
            if(mainmenu == 4) {
                nummenuitems = 10;
                if(keyselect != 0)
                    sprintf(menustring[0], "Forwards: %s", SDL_GetScancodeName(forwardkey));
                else
                    sprintf(menustring[0], "Forwards: _");
                startx[0] = 10;
                starty[0] = 400;
                endx[0] = startx[0] + strlen(menustring[0]) * 10;
                endy[0] = starty[0] + 20;
                ...
                if(keyselect != 1)
                    sprintf(menustring[1], "Back: %s", SDL_GetScancodeName(backkey));
                else
                    sprintf(menustring[1], "Back: _");
                startx[1] = 10 + 40;
                starty[1] = 360;
                endx[1] = startx[1] + strlen(menustring[1]) * 10;
                endy[1] = starty[1] + 20;
                ...
                if(keyselect != 2)
                    sprintf(menustring[2], "Left: %s", SDL_GetScancodeName(leftkey));
                else
                    sprintf(menustring[2], "Left: _");
                startx[2] = 10 + 40;
                starty[2] = 320;
                endx[2] = startx[2] + strlen(menustring[2]) * 10;
                endy[2] = starty[2] + 20;
                ...
                if(keyselect != 3)
                    sprintf(menustring[3], "Right: %s", SDL_GetScancodeName(rightkey));
                else
                    sprintf(menustring[3], "Right: _");
                startx[3] = 10 + 30;
                starty[3] = 280;
                endx[3] = startx[3] + strlen(menustring[3]) * 10;
                endy[3] = starty[3] + 20;
                ...
                if(keyselect != 4)
                    sprintf(menustring[4], "Crouch: %s", SDL_GetScancodeName(crouchkey));
                else
                    sprintf(menustring[4], "Crouch: _");
                startx[4] = 10 + 20;
                starty[4] = 240;
                endx[4] = startx[4] + strlen(menustring[4]) * 10;
                endy[4] = starty[4] + 20;
                ...
                if(keyselect != 5)
                    sprintf(menustring[5], "Jump: %s", SDL_GetScancodeName(jumpkey));
                else
                    sprintf(menustring[5], "Jump: _");
                startx[5] = 10 + 40;
                starty[5] = 200;
                endx[5] = startx[5] + strlen(menustring[5]) * 10;
                endy[5] = starty[5] + 20;
                ...
                if(keyselect != 6)
                    sprintf(menustring[6], "Draw: %s", SDL_GetScancodeName(drawkey));
                else
                    sprintf(menustring[6], "Draw: _");
                startx[6] = 10 + 40;
                starty[6] = 160;
                endx[6] = startx[6] + strlen(menustring[6]) * 10;
                endy[6] = starty[6] + 20;
                ...
                if(keyselect != 7)
                    sprintf(menustring[7], "Throw: %s", SDL_GetScancodeName(throwkey));
                else
                    sprintf(menustring[7], "Throw: _");
                startx[7] = 10 + 30;
                starty[7] = 120;
                endx[7] = startx[7] + strlen(menustring[7]) * 10;
                endy[7] = starty[7] + 20;
                ...
                if(keyselect != 8)
                    sprintf(menustring[8], "Attack: %s", getButtonName(attackbutton));
                else
                    sprintf(menustring[8], "Attack: _");
                startx[8] = 10 + 20;
                starty[8] = 80;
                endx[8] = startx[8] + strlen(menustring[8]) * 10;
                endy[8] = starty[8] + 20;
            ...
            if(mainmenu == 5) {
                ...
                if(campaignchoicenum > 0)
                    for(i = accountcampaignchoicesmade[accountactive];
                        i < accountcampaignchoicesmade[accountactive] + campaignchoicenum;
                        i++) {
                        sprintf(menustring[7 + i], "%s", campaigndescription[levelorder[i]]);
                        startx[7 + i] =
                            30 + 120 +
                            campaignlocationx[campaignchoicewhich[i - (accountcampaignchoicesmade[accountactive])]] *
                                400 / 512;
                        starty[7 + i] =
                            30 + 30 +
                            (512 -
                             campaignlocationy[campaignchoicewhich[i - (accountcampaignchoicesmade[accountactive])]]) *
                                400 / 512;
                        endx[7 + i] = startx[7 + i] + 10;
                        endy[7 + i] = starty[7 + i] + 10;
                        movex[7 + i] = 0;
            ...
            if(mainmenu == 9) {
                ...
                for(j = 0; j < numchallengelevels; j++) {
                    ...
                    sprintf(
                        temp,
                        "%d:",
                        (int)(((int)accountfasttime[accountactive][j] - (int)(accountfasttime[accountactive][j]) % 60) /
                              60));
                    strcat(menustring[j], temp);
                    if((int)(accountfasttime[accountactive][j]) % 60 < 10)
                        strcat(menustring[j], "0");
                ...
                endx[numchallengelevels + 1] =
                    startx[numchallengelevels + 1] + strlen(menustring[numchallengelevels + 1]) * 10;
        ...
        if(mainmenu == 1 || mainmenu == 2)
            for(i = 1; i < 4; i++) {
                if((mousecoordh / screenwidth * 640) > startx[i] && (mousecoordh / screenwidth * 640) < endx[i] &&
                   480 - (mousecoordv / screenheight * 480) > starty[i] &&
                   480 - (mousecoordv / screenheight * 480) < endy[i]) {
                    selected = i;
                }
            }

        if(mainmenu == 3 || mainmenu == 4 || mainmenu == 5 || mainmenu == 6 || mainmenu == 7 || mainmenu == 8 ||
           mainmenu == 9 || mainmenu == 10 || mainmenu == 11 || mainmenu == 13 || mainmenu == 17)
            for(i = 0; i < nummenuitems; i++) {
                if((mousecoordh / screenwidth * 640) > startx[i] && (mousecoordh / screenwidth * 640) < endx[i] &&
                   ...
                   480 - (mousecoordv / screenheight * 480) < endy[i]) {
                    if(mainmenu != 5)
                        selected = i;
                    if(mainmenu == 5 && (i != 0 && i != 6))
                        selected = i;
                    if(mainmenu == 9 && (i != numchallengelevels + 1))
                        selected = i;
                    if(mainmenu == 11 && (i != numchallengelevels + 1))
                        selected = i;
                }
            }
        ...
        if(nummenuitems > 0) {
            for(j = 0; j < nummenuitems; j++) {
                if(j <= 3 || (mainmenu != 1 && mainmenu != 2)) {
                    ...
                    if(mainmenu == 1 || mainmenu == 2) {
                        ...
                        for(i = 0; i < 10; i++) {
                            if(1 - ((float)i) / 10 - (1 - selectedlong[j]) > 0) {
                                ...
                                glBegin(GL_QUADS);
                                glTexCoord2f(0, 0);
                                glVertex3f(startx[j] - ((float)i) * 1 / 2 + offsetx[j] * ((float)i) / 2 +
                                               movex[j] * transition,
                                           starty[j] - ((float)i) * 1 / 2 + offsety[j] * ((float)i) / 2 +
                                               movey[j] * transition,
                                           0.0f);
                                glTexCoord2f(1, 0);
                                glVertex3f(endx[j] + ((float)i) * 1 / 2 + offsetx[j] * ((float)i) / 2 +
                                               movex[j] * transition,
                                           starty[j] - ((float)i) * 1 / 2 + offsety[j] * ((float)i) / 2 +
                                               movey[j] * transition,
                                           0.0f);
                                glTexCoord2f(1, 1);
                                glVertex3f(
                                    endx[j] + ((float)i) * 1 / 2 + offsetx[j] * ((float)i) / 2 + movex[j] * transition,
                                    endy[j] + ((float)i) * 1 / 2 + offsety[j] * ((float)i) / 2 + movey[j] * transition,
                                    0.0f);
                                glTexCoord2f(0, 1);
                                glVertex3f(startx[j] - ((float)i) * 1 / 2 + offsetx[j] * ((float)i) / 2 +
                                               movex[j] * transition,
                                           endy[j] + ((float)i) * 1 / 2 + offsety[j] * ((float)i) / 2 +
                                               movey[j] * transition,
                                           0.0f);
                                glEnd();
                                glPopMatrix();
                    ...
                    if(mainmenu == 3 || mainmenu == 4 || mainmenu == 5 || mainmenu == 6 || mainmenu == 7 ||
                       mainmenu == 8 || mainmenu == 9 || mainmenu == 10 || mainmenu == 11 || mainmenu == 13 ||
                       mainmenu == 17) {
                        if(mainmenu != 5 || j < 6) {
                            glColor4f(1, 0, 0, 1);
                            if(mainmenu == 9 && j > accountprogress[accountactive] && j < numchallengelevels)
                                glColor4f(0.5, 0, 0, 1);
                            if(mainmenu == 11 && j > accountprogress[accountactive] && j < numchallengelevels)
                                glColor4f(0.5, 0, 0, 1);
                            // if(1-((float)i)/10-(1-selectedlong[j])>0){
                            glMatrixMode(GL_MODELVIEW); // Select The Modelview Matrix
                            glPushMatrix();
                            if(mainmenu != 7 || j != 0 || !entername)
                                ...
                                if(displayblink) {
                                    sprintf(string, "_");
                                    text.glPrint(
                                        startx[j] + (float)(displayselected) * 10, starty[j], string, 0, 1, 640, 480);
                                }
                                k = 0;
                            ...
                            for(i = 0; i < 15; i++) {
                                if(1 - ((float)i) / 15 - (1 - selectedlong[j]) > 0) {
                                    ...
                                    if(mainmenu == 9 && j > accountprogress[accountactive] && j < numchallengelevels)
                                        glColor4f(0.5, 0, 0, (1 - ((float)i) / 10 - (1 - selectedlong[j])) * .25);
                                    if(mainmenu == 11 && j > accountprogress[accountactive] && j < numchallengelevels)
                                        glColor4f(0.5, 0, 0, (1 - ((float)i) / 10 - (1 - selectedlong[j])) * .25);
                                    if(mainmenu == 3)
                                        text.glPrint(startx[j] - ((float)i) + offsetx[j] * ((float)i) / 4 -
                                                         ((/*1*/ +((float)i) / 70) * strlen(menustring[j])) * 3,
                                                     starty[j] /*-i*1/2*/ + offsety[j] * ((float)i) / 4,
                                                     menustring[j],
                                                     0,
                                                     1 + ((float)i) / 70,
                                                     640,
                                                     480);
                                    if(mainmenu == 4)
                                        text.glPrint(startx[j] - ((float)i) +
                                                         offsetx[j] * ((float)i) /
                                                             4 /*-((((float)i)/70)*strlen(menustring[j]))*3*/,
                                                     starty[j] /*-i*1/2*/ + offsety[j] * ((float)i) / 4,
                                                     menustring[j],
                                                     0,
                                                     1 + ((float)i) / 70,
                            ...
                            if(j > 6 && j < nummenuitems - 1) {
                                ...
                                if(j >= 6 + accountcampaignchoicesmade[accountactive]) {
                                    linestart.x = (startx[6 + accountcampaignchoicesmade[accountactive]] +
                                                   endx[6 + accountcampaignchoicesmade[accountactive]]) /
                                                  2;
                                    linestart.y = (starty[6 + accountcampaignchoicesmade[accountactive]] +
                                                   endy[6 + accountcampaignchoicesmade[accountactive]]) /
                                                  2;
                                }
                            ...
                            if(j > 6 && (j - 7 < accountcampaignchoicesmade[accountactive]))
                                itemsize *= .5;
                            if(!(j - 7 > accountcampaignchoicesmade[accountactive] + campaignchoicenum)) {
                                glMatrixMode(GL_MODELVIEW); // Select The Modelview Matrix
                                glPushMatrix();
                                glBegin(GL_QUADS);
                                glTexCoord2f(0, 0);
                                glVertex3f(midpoint.x - itemsize + movex[j] * transition,
                                           midpoint.y - itemsize + movey[j] * transition,
                                           0.0f);
                                glTexCoord2f(1, 0);
                                glVertex3f(midpoint.x + itemsize + movex[j] * transition,
                                           midpoint.y - itemsize + movey[j] * transition,
                                ...
                                for(i = 0; i < 10; i++) {
                                    if(1 - ((float)i) / 10 - (1 - selectedlong[j]) > 0) {
                                        ...
                                        glBegin(GL_QUADS);
                                        glTexCoord2f(0, 0);
                                        glVertex3f(midpoint.x - itemsize - ((float)i) * 1 / 2 +
                                                       offsetx[j] * ((float)i) / 2 + movex[j] * transition,
                                                   midpoint.y - itemsize - ((float)i) * 1 / 2 +
                                                       offsety[j] * ((float)i) / 2 + movey[j] * transition,
                                                   0.0f);
                                        glTexCoord2f(1, 0);
                                        glVertex3f(midpoint.x + itemsize + ((float)i) * 1 / 2 +
                                                       offsetx[j] * ((float)i) / 2 + movex[j] * transition,
                            ...
                            if(j - 7 >= accountcampaignchoicesmade[accountactive]) {
                                ...
                                text.glPrintOutlined(
                                    0.9, 0, 0, startx[j] + 10, starty[j] - 4, menustring[j], 0, 0.6, 640, 480);
                                glDisable(GL_DEPTH_TEST);
                            }
                        }
                    }
