...
void Weapons::DoStuff()
{
    ...
    for(i = 0; i < numweapons; i++) {
        ...
        if(damage[i] >= 2 && type[i] == staff && owner[i] != -1) {
            ...
            tempvel+=speed;
            sprites.MakeSprite(cloudimpactsprite, position[i]+(tippoint[i]-position[i])*((float)j-2)/8,tempvel*.5,
            115/255,73/255,12/255, .15+float(abs(rand()%100)-50)/1000, .7);
            }*/
            for(j = 0; j < 40; j++) {
                tempvel.x = float(abs(rand() % 100) - 50) / 20;
                tempvel.y = float(abs(rand() % 100) - 50) / 20;
                ...
                sprites.MakeSprite(splintersprite,
                                   position[i] + (tippoint[i] - position[i]) * ((float)j - 8) / 32,
                                   tempvel * .5,
                                   115 / 255,
                                   73 / 255,
                                   12 / 255,
            ...
            if(tempowner != -1) {
                ...
                if(player[tempowner].numweapons) {
                    ...
                    if(player[tempowner].weaponstuck == player[tempowner].numweapons)
        ...
        if(owner[i] == -1 && (velocity[i].x || velocity[i].y || velocity[i].z) && !physics[i]) {
            ...
            if(whichpatchx > 0 && whichpatchz > 0 && whichpatchx < subdivision && whichpatchz < subdivision)
                if(terrain.patchobjectnum[whichpatchx][whichpatchz]) {
                    for(j = 0; j < terrain.patchobjectnum[whichpatchx][whichpatchz]; j++) {
                        ...
                        whichhit = objects.model[k].LineCheck(
                            &start, &end, &colpoint, &objects.position[k], &objects.rotation[k]);
                        if(whichhit != -1) {
                            if(objects.type[k] == treetrunktype) {
                                objects.model[k].MakeDecal(
                                    breakdecal,
                                    DoRotation(colpoint - objects.position[k], 0, -objects.rotation[k], 0),
                                    .1,
                                    1,
                                    rand() % 360);
                                normalrot =
                                    DoRotation(objects.model[k].facenormals[whichhit], 0, objects.rotation[k], 0);
                                velocity[i] = 0;
                                if(type[i] == knife)
                                    position[i] = colpoint - normalrot * .1;
                                if(type[i] == sword)
                                    position[i] = colpoint - normalrot * .2;
                                if(type[i] == staff)
                                    position[i] = colpoint - normalrot * .2;
            ...
            if(velocity[i].x || velocity[i].y || velocity[i].z)
                for(j = 0; j < numplayers; j++) {
                    footvel = 0;
                    footpoint =
                        DoRotation((player[j].skeleton.joints[player[j].skeleton.jointlabels[abdomen]].position +
                                    player[j].skeleton.joints[player[j].skeleton.jointlabels[neck]].position) /
                                       2,
                                   0,
                                   player[j].rotation,
                                   0) *
                            player[j].scale +
                        player[j].coords;
                    if(owner[i] == -1 && findDistancefastflat(&position[i], &player[j].coords) < 1.5 &&
                       findDistancefast(&position[i], &player[j].coords) < 4 && player[j].weaponstuck == -1 &&
                       !player[j].skeleton.free && j != oldowner[i]) {
                        if((player[j].aitype != attacktypecutoff || abs(rand() % 6) == 0 ||
                           ...
                           !missed[i]) {
                            ...
                            if((player[j].creature == wolftype && rand() % 3 != 0 && player[j].weaponactive == -1 &&
                               ...
                               (player[j].creature == rabbittype && rand() % 2 == 0 &&
                                player[j].aitype == attacktypecutoff && player[j].weaponactive == -1)) {
                                float gLoc[3];
                                float vel[3];
                                ...
                                player[j].RagDoll(0);
                                player[j].skeleton.joints[player[j].skeleton.jointlabels[abdomen]].velocity +=
                                    velocity[i] * 2;
                                player[j].skeleton.joints[player[j].skeleton.jointlabels[neck]].velocity +=
                                    velocity[i] * 2;
                                player[j].skeleton.joints[player[j].skeleton.jointlabels[rightshoulder]].velocity +=
                                    velocity[i] * 2;
                                player[j].skeleton.joints[player[j].skeleton.jointlabels[leftshoulder]].velocity +=
                                    velocity[i] * 2;
                                ...
                                if(tutoriallevel == 1)
                                    sprites.MakeSprite(cloudimpactsprite, footpoint, footvel, 1, 1, 1, .8, .3);
                                footvel = tippoint[i] - position[i];
                                Normalise(&footvel);
                                if(config.video_blood && tutoriallevel != 1)
                                    sprites.MakeSprite(bloodflamesprite, footpoint, footvel * -1, 1, 0, 0, .6, 1);
            ...
            if(position[i].y < terrain.getHeight(position[i].x, position[i].z)) {
                if(terrain.getOpacity(position[i].x, position[i].z) < .2) {
                    ...
                    if(environment == snowyenvironment) {
                        if(findDistancefast(&position[i], &viewer) < viewdistance * viewdistance / 4)
        ...
        for(int l = 0; l < 10; l++) {
            if(owner[i] == -1 && (velocity[i].x || velocity[i].y || velocity[i].z) && physics[i]) {
                ...
                if(!freeze) {
                    if(freetime[i] > .04)
                        velocity[i] = velocity[i] + (newpoint1 - position[i]) / multiplier;
                    if(freetime[i] > .04)
                        tipvelocity[i] = tipvelocity[i] + (newpoint2 - tippoint[i]) / multiplier;
                }
                    ...
                    if(terrain.patchobjectnum[whichpatchx][whichpatchz]) {
                        for(j = 0; j < terrain.patchobjectnum[whichpatchx][whichpatchz]; j++) {
                            ...
                            if(whichhit != -1) {
                                ...
                                position[i] = colpoint;
                                terrainnormal =
                                    DoRotation(objects.model[k].facenormals[whichhit], 0, objects.rotation[k], 0) * -1;
                                ReflectVector(&velocity[i], &terrainnormal);
                                position[i] += terrainnormal * .002;

                                bounceness = terrainnormal * findLength(&velocity[i]) *
                                             (abs(normaldotproduct(velocity[i], terrainnormal)));
                            ...
                            if(whichhit != -1) {
                                ...
                                bounceness = terrainnormal * findLength(&tipvelocity[i]) *
                                             (abs(normaldotproduct(tipvelocity[i], terrainnormal)));
                                if(findLengthfast(&tipvelocity[i]) < findLengthfast(&bounceness))
                                    bounceness = 0;
                                frictionness = abs(normaldotproduct(tipvelocity[i], terrainnormal));
                                tipvelocity[i] -= bounceness;
                                if(1 - friction * frictionness > 0)
                                    tipvelocity[i] *= 1 - friction * frictionness;
                                else
                                    tipvelocity[i] = 0;
                            ...
                            if((objects.type[k] != boxtype && objects.type[k] != platformtype &&
                                objects.type[k] != walltype && objects.type[k] != weirdtype) ||
                               objects.rotation2[k] != 0)
                                for(m = 0; m < 2; m++) {
                                    mid =
                                        (position[i] * (21 + (float)m * 10) + tippoint[i] * (19 - (float)m * 10)) / 40;
                                    oldmid2 = mid;
                                    oldmid = (oldposition[i] * (21 + (float)m * 10) +
                                              oldtippoint[i] * (19 - (float)m * 10)) /
                                             40;
                                    ...
                                    if(whichhit != -1) {
                                        ...
                                        terrainnormal =
                                            DoRotation(
                                                objects.model[k].facenormals[whichhit], 0, objects.rotation[k], 0) *
                                            -1;
                                        ReflectVector(&velocity[i], &terrainnormal);
                                    ...
                                    mid =
                                        (position[i] * (19 - (float)m * 10) + tippoint[i] * (21 + (float)m * 10)) / 40;
                                    oldmid2 = mid;
                                    oldmid = (oldposition[i] * (19 - (float)m * 10) +
                                              oldtippoint[i] * (21 + (float)m * 10)) /
                                             40;
                                ...
                                if(whichhit != -1) {
                                    ...
                                    point[0] =
                                        DoRotation(
                                            objects.model[k].vertex[objects.model[k].Triangles[whichhit].vertex[0]],
                                            0,
                                            objects.rotation[k],
                                            0) +
                                        objects.position[k];
                                    point[1] =
                                        DoRotation(
                                            objects.model[k].vertex[objects.model[k].Triangles[whichhit].vertex[1]],
                                            0,
                                            objects.rotation[k],
                                            0) +
                                        objects.position[k];
                                    point[2] =
                                        DoRotation(
                                            objects.model[k].vertex[objects.model[k].Triangles[whichhit].vertex[2]],
                                            0,
                                            objects.rotation[k],
                                            0) +
                                        objects.position[k];
                                    if(DistancePointLine(
                                           &closestswordpoint, &point[0], &point[1], &distance, &colpoint))
                                    ...
                                    if(DistancePointLine(
                                           &closestswordpoint, &point[1], &point[2], &distance, &colpoint))
                                        if(distance < closestdistance || closestdistance == -1) {
                                            closestpoint = colpoint;
                                            closestdistance = distance;
                                            closestline = 1;
                                    ...
                                    if(DistancePointLine(
                                           &closestswordpoint, &point[2], &point[0], &distance, &colpoint))
                                        if(distance < closestdistance || closestdistance == -1) {
                                            closestpoint = colpoint;
                                            closestdistance = distance;
                                            closestline = 2;
                                    ...
                                    if(closestdistance != -1 && isnormal(closestdistance)) {
                                        if(DistancePointLine(
                                               &closestpoint, &position[i], &tippoint[i], &distance, &colpoint)) {
                ...
                if(whichhit != -1 || position[i].y < terrain.getHeight(position[i].x, position[i].z)) {
                    ...
                    position[i] += terrainnormal * .002;
                    bounceness =
                        terrainnormal * findLength(&velocity[i]) * (abs(normaldotproduct(velocity[i], terrainnormal)));
                    if(findLengthfast(&velocity[i]) < findLengthfast(&bounceness))
                        bounceness = 0;
                    frictionness = abs(normaldotproduct(velocity[i], terrainnormal));
                    velocity[i] -= bounceness;
                    if(1 - friction * frictionness > 0)
                        ...
                        velocity[i] = 0;
                    if(terrain.getOpacity(position[i].x, position[i].z) < .2)
                        velocity[i] += bounceness * elasticity * .3;
                    else
                        velocity[i] += bounceness * elasticity;
                ...
                if(whichhit != -1 || tippoint[i].y < terrain.getHeight(tippoint[i].x, tippoint[i].z)) {
                    ...
                    if(terrain.getOpacity(tippoint[i].x, tippoint[i].z) < .2)
                        tipvelocity[i] += bounceness * elasticity * .3;
                    else
                        tipvelocity[i] += bounceness * elasticity;
                    // if (type[i]==knife) printf("tipvelocity of knife %d now %f,%f,%f.\n", i, tipvelocity[i].x,
                    // tipvelocity[i].y, tipvelocity[i].z);
                    ...
                    if(findLengthfast(&bounceness) > 1) {
                        ...
                        if(terrain.getOpacity(tippoint[i].x, tippoint[i].z) < .2) {
                            ...
                            if(environment == snowyenvironment) {
                                if(findDistancefast(&tippoint[i], &viewer) < viewdistance * viewdistance / 4)
                                    sprites.MakeSprite(cloudsprite,
                                                       tippoint[i],
                                                       tipvelocity[i],
                                                       terrainlight.x,
                ...
                PlaySoundEx( whichsound, samp[whichsound], NULL, true);
                OPENAL_3D_SetAttributes(channels[whichsound], gLoc, vel);
                if(terrain.getOpacity(position[i].x,position[i].z)>.2)OPENAL_SetVolume(channels[whichsound],
                128*findLengthfast(&bounceness));
                ...
                PlaySoundEx( whichsound, samp[whichsound], NULL, true);
                OPENAL_3D_SetAttributes(channels[whichsound], gLoc, vel);
                if(terrain.getOpacity(tippoint[i].x,tippoint[i].z)>.2)OPENAL_SetVolume(channels[whichsound],
                128*findLengthfast(&bounceness));
                else OPENAL_SetVolume(channels[whichsound], 32*findLengthfast(&bounceness));
                OPENAL_SetPaused(channels[whichsound], false);
                }
        ...
        if(blooddrip[i] && bloody[i]) {
            ...
            if(blooddrip[i] > 5)
                blooddrip[i] = 5;
            if(blooddripdelay[i] < 0 && config.video_blood) {
                blooddripdelay[i] = 1;
                XYZ bloodvel;
                XYZ bloodloc;
        ...
        if(onfire[i]) {
            ...
            if(onfire[i] && flamedelay[i] <= 0) {
                ...
                sprites.MakeSprite(weaponflamesprite,
                                   position[i] + tippoint[i] * (((float)abs(rand() % 100)) / 600 + .05),
                                   normalrot,
                                   1,
                                   1,
                                   1,
                                   (.6 + (float)abs(rand() % 100) / 200 - .25) * 1 / 3,
                                   1);
        ...
        if(!onfire[i] && owner[i] == -1 && type[i] != staff) {
            ...
            if(flamedelay[i] <= 0) {
                ...
                if(rand() % 50 == 0 && findDistancefast(&position[i], &viewer) > 80) {
                    ...
                    sprites.MakeSprite(weaponshinesprite,
                                       shinepoint,
                                       normalrot,
                                       1,
                                       1,
                                       1,
                                       (.1 + (float)abs(rand() % 100) / 200 - .25) * 1 / 3 *
                                           fast_sqrt(findDistance(&shinepoint, &viewer)),
                                       1);
                    sprites.speed[sprites.numsprites - 1] = 4;
                    sprites.alivetime[sprites.numsprites - 1] = .3;
                }
...
int Weapons::Draw()
{
    ...
    glDepthMask(1);
    for(i = 0; i < numweapons; i++) {
        if((frustum.SphereInFrustum(position[i].x, position[i].y, position[i].z, 1) &&
            findDistancefast(&viewer, &position[i]) < viewdistance * viewdistance)) {
            draw = 0;
            if(owner[i] == -1) {
                draw = 1;
                if(velocity[i].x && !physics[i])
                    drawhowmany[i] = 10;
                else
                    drawhowmany[i] = 1;
            }
            if(owner[i] != -1) {
                if(player[owner[i]].occluded < 25)
                    if((frustum.SphereInFrustum(player[owner[i]].coords.x,
                                                player[owner[i]].coords.y + player[owner[i]].scale * 3,
                                                player[owner[i]].coords.z,
                                                player[owner[i]].scale * 8) &&
                        findDistancefast(&viewer, &player[owner[i]].coords) < viewdistance * viewdistance) ||
                       player[owner[i]].skeleton.free == 3)
                        draw = 1;
                if((player[owner[i]].targetanimation == knifeslashstartanim ||
                    player[owner[i]].targetanimation == swordsneakattackanim ||
                    (player[owner[i]].currentanimation == staffhitanim && player[owner[i]].currentframe > 1) ||
                    (player[owner[i]].currentanimation == staffhitreversedanim && player[owner[i]].currentframe > 1) ||
                    (player[owner[i]].currentanimation == staffspinhitanim && player[owner[i]].currentframe > 1) ||
                    (player[owner[i]].currentanimation == staffspinhitreversedanim &&
                     player[owner[i]].currentframe > 1) ||
                    (player[owner[i]].currentanimation == staffgroundsmashanim && player[owner[i]].currentframe > 1) ||
                    (player[owner[i]].targetanimation == swordslashanim && player[owner[i]].targetframe < 7) ||
                    player[owner[i]].targetanimation == crouchstabanim ||
                    player[owner[i]].targetanimation == swordslashreversalanim ||
                    player[owner[i]].targetanimation == swordslashreversedanim ||
                    player[owner[i]].targetanimation == knifefollowanim ||
                    player[owner[i]].targetanimation == swordgroundstabanim ||
                    player[owner[i]].targetanimation == knifethrowanim) &&
                   player[owner[i]].targetanimation == lastdrawnanim[i] && !player[owner[i]].skeleton.free) {
                    drawhowmany[i] = 10;
                } else
                    drawhowmany[i] = 1;
            ...
            if(draw) {
                ...
                for(j = drawhowmany[i]; j > 0; j--) {
                    ...
                    if(owner[i] != -1)
                        glTranslatef(position[i].x * (((float)(j)) / drawhowmany[i]) +
                                         lastdrawnposition[i].x * (1 - ((float)(j)) / drawhowmany[i]),
                                     position[i].y * (((float)(j)) / drawhowmany[i]) - .02 +
                                         lastdrawnposition[i].y * (1 - ((float)(j)) / drawhowmany[i]),
                                     position[i].z * (((float)(j)) / drawhowmany[i]) +
                                         lastdrawnposition[i].z * (1 - ((float)(j)) / drawhowmany[i]));
                    if(owner[i] == -1)
                        glTranslatef(position[i].x * (((float)(j)) / drawhowmany[i]) +
                                         lastdrawnposition[i].x * (1 - ((float)(j)) / drawhowmany[i]),
                                     position[i].y * (((float)(j)) / drawhowmany[i]) +
                                         lastdrawnposition[i].y * (1 - ((float)(j)) / drawhowmany[i]),
                                     position[i].z * (((float)(j)) / drawhowmany[i]) +
                                         lastdrawnposition[i].z * (1 - ((float)(j)) / drawhowmany[i]));
                    // glTranslatef(position[i].x,position[i].y-.02,position[i].z);
                    glRotatef(bigrotation[i] * (((float)(j)) / drawhowmany[i]) +
                                  lastdrawnbigrotation[i] * (1 - ((float)(j)) / drawhowmany[i]),
                              0,
                              1,
                              0);
                    glRotatef(bigtilt2[i] * (((float)(j)) / drawhowmany[i]) +
                                  lastdrawnbigtilt2[i] * (1 - ((float)(j)) / drawhowmany[i]),
                              1,
                              0,
                              0);
                    glRotatef(bigtilt[i] * (((float)(j)) / drawhowmany[i]) +
                                  lastdrawnbigtilt[i] * (1 - ((float)(j)) / drawhowmany[i]),
                              0,
                              0,
                              1);
                    glRotatef(-rotation1[i] * (((float)(j)) / drawhowmany[i]) -
                                  lastdrawnrotation1[i] * (1 - ((float)(j)) / drawhowmany[i]) + 90,
                              0,
                              1,
                              0);
                    glRotatef(-rotation2[i] * (((float)(j)) / drawhowmany[i]) -
                                  lastdrawnrotation2[i] * (1 - ((float)(j)) / drawhowmany[i]) + 90,
                              0,
                              0,
                              1);
                    glRotatef(-rotation3[i] * (((float)(j)) / drawhowmany[i]) -
                                  lastdrawnrotation3[i] * (1 - ((float)(j)) / drawhowmany[i]),
                              0,
                              1,
                              0);
                    glRotatef(smallrotation[i] * (((float)(j)) / drawhowmany[i]) +
                                  lastdrawnsmallrotation[i] * (1 - ((float)(j)) / drawhowmany[i]),
                              1,
                              0,
                              0);
                    glRotatef(smallrotation2[i] * (((float)(j)) / drawhowmany[i]) +
                                  lastdrawnsmallrotation2[i] * (1 - ((float)(j)) / drawhowmany[i]),
                              0,
                              1,
                              0);

                    if(owner[i] != -1) {
                        if(player[owner[i]].targetanimation == staffhitanim ||
                           player[owner[i]].currentanimation == staffhitanim ||
                           player[owner[i]].targetanimation == staffhitreversedanim ||
                           player[owner[i]].currentanimation == staffhitreversedanim) {
                            glTranslatef(0, 0, -.3);
                        }
                        if(player[owner[i]].targetanimation == staffgroundsmashanim ||
                           player[owner[i]].currentanimation == staffgroundsmashanim ||
                           player[owner[i]].targetanimation == staffspinhitreversedanim ||
                           player[owner[i]].currentanimation == staffspinhitreversedanim ||
                           player[owner[i]].targetanimation == staffspinhitanim ||
                           player[owner[i]].currentanimation == staffspinhitanim) {
                            glTranslatef(0, 0, -.1);
                        }
                    }
                    /*if(type[i]==knife){
                    ...
                    if(type[i] == knife) {
                        ...
                        if(!bloody[i] || !config.video_blood)
                ...
                lastdrawnsmallrotation2[i] = smallrotation2[i];
                if(owner[i] != -1)
                    lastdrawnanim[i] = player[owner[i]].currentanimation;
            }
            if(owner[i] != -1) {
                glMatrixMode(GL_MODELVIEW); // Select The Modelview Matrix
                glPushMatrix();
...
{
    if(stafftextureptr)
        glDeleteTextures(1, &stafftextureptr);
    if(knifetextureptr)
        glDeleteTextures(1, &knifetextureptr);
    if(lightbloodknifetextureptr)
        glDeleteTextures(1, &lightbloodknifetextureptr);
    if(bloodknifetextureptr)
        glDeleteTextures(1, &bloodknifetextureptr);
    if(swordtextureptr)
        glDeleteTextures(1, &swordtextureptr);
    if(lightbloodswordtextureptr)
        glDeleteTextures(1, &lightbloodswordtextureptr);
    if(bloodswordtextureptr)
        glDeleteTextures(1, &bloodswordtextureptr);
}
