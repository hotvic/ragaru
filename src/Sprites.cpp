...
void Sprites::Draw()
{
    ...
    for(i = 0; i < numsprites; i++) {
        ...
        if(type[i] == splintersprite && (lasttype != type[i] || lastspecial != special[i])) {
            if(special[i] == 0)
                glBindTexture(GL_TEXTURE_2D, splintertexture);
            if(special[i] == 1)
                glBindTexture(GL_TEXTURE_2D, leaftexture);
            if(special[i] == 2)
                glBindTexture(GL_TEXTURE_2D, snowflaketexture);
            if(special[i] == 3)
                glBindTexture(GL_TEXTURE_2D, toothtexture);
            if(!blend) {
                blend = 1;
                glAlphaFunc(GL_GREATER, 0.0001);
                glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        ...
        if(type[i] != snowsprite)
            distancemult =
                (viewdistsquared -
                 (findDistancefast(&viewer, &position[i]) - (viewdistsquared * fadestart)) * (1 / (1 - fadestart))) /
                viewdistsquared;
        if(type[i] == snowsprite)
            distancemult =
                (144 - (findDistancefast(&tempviewer, &position[i]) - (144 * fadestart)) * (1 / (1 - fadestart))) / 144;
        if(type[i] != flamesprite) {
            if(distancemult >= 1)
                glColor4f(
                    color[i][0] * lightcolor[0], color[i][1] * lightcolor[1], color[i][2] * lightcolor[2], opacity[i]);
            if(distancemult < 1)
                glColor4f(color[i][0] * lightcolor[0],
                          color[i][1] * lightcolor[1],
                          color[i][2] * lightcolor[2],
                          opacity[i] * distancemult);
        }
        if(type[i] == flamesprite) {
            if(distancemult >= 1)
                glColor4f(color[i][0], color[i][1], color[i][2], opacity[i]);
            if(distancemult < 1)
                glColor4f(color[i][0], color[i][1], color[i][2], opacity[i] * distancemult);
        }
        lasttype = type[i];
        ...
        if((type[i] == flamesprite || type[i] == weaponflamesprite || type[i] == weaponshinesprite ||
            type[i] == bloodflamesprite)) {
            if(alivetime[i] < .14)
                glScalef(alivetime[i] / .14, alivetime[i] / .14, alivetime[i] / .14);
        }
            ...
            if(alivetime[i] < .3) {
                if(distancemult >= 1)
                    glColor4f(color[i][0] * lightcolor[0],
                              color[i][1] * lightcolor[1],
                              color[i][2] * lightcolor[2],
                              opacity[i] * alivetime[i] / .3);
                if(distancemult < 1)
                    glColor4f(color[i][0] * lightcolor[0],
                              color[i][1] * lightcolor[1],
                              color[i][2] * lightcolor[2],
                              opacity[i] * distancemult * alivetime[i] / .3);
            }
        }
            ...
            if(alivetime[i] < .2) {
                ...
                if(distancemult < 1)
                    glColor4f(color[i][0] * lightcolor[0],
                              color[i][1] * lightcolor[1],
                              color[i][2] * lightcolor[2],
                              distancemult * alivetime[i] / .2);
            } else {
                if(distancemult >= 1)
                    glColor4f(color[i][0] * lightcolor[0], color[i][1] * lightcolor[1], color[i][2] * lightcolor[2], 1);
                if(distancemult < 1)
                    glColor4f(color[i][0] * lightcolor[0], color[i][1] * lightcolor[1], color[i][2] * lightcolor[2], 1);
            }
        }
        ...
        glVertex3f(.5 * size[i], .5 * size[i], 0.0f);
    ...
    for(i = numsprites - 1; i >= 0; i--) {
        ...
        if(type[i] != snowsprite)
            velocity[i] += windvector * multiplier;
        if(type[i] == flamesprite || type[i] == smoketype)
            position[i] += windvector * multiplier / 2;
        if((type[i] == flamesprite || type[i] == weaponflamesprite || type[i] == weaponshinesprite ||
            type[i] == bloodflamesprite))
            multiplier *= speed[i] * .7;
        ...
        if(type[i] == breathsprite) {
            opacity[i] -= multiplier / 2;
            size[i] += multiplier / 2;
            if(findLength(&velocity[i]) <= multiplier)
                velocity[i] = 0;
            else {
                XYZ slowdown;
        ...
        if(type[i] == snowsprite) {
            ...
            if(position[i].y < tempviewer.y - 6)
                position[i].y += 12;
            if(position[i].y > tempviewer.y + 6)
                position[i].y -= 12;
            if(position[i].z < tempviewer.z - 6)
                position[i].z += 12;
            if(position[i].z > tempviewer.z + 6)
                position[i].z -= 12;
            if(position[i].x < tempviewer.x - 6)
                position[i].x += 12;
            if(position[i].x > tempviewer.x + 6)
                position[i].x -= 12;
        }
                ...
                for(j = 0; j < numplayers; j++) {
                    if(!spritehit && player[j].dead && alivetime[i] > .1) {
                        ...
                        if(!player[j].skeleton.free)
                            where = DoRotation(where, 0, -player[j].rotation, 0);
                        startpoint = where;
                        where = position[i];
                        where -= player[j].coords;
                        if(!player[j].skeleton.free)
                        ...
                        movepoint = 0;
                        rotationpoint = 0;
                        whichtri = player[j].skeleton.drawmodel.LineCheck(
                    ...
                    if(terrain.patchobjectnum[whichpatchx][whichpatchz]) {
                        if(!spritehit)
                            for(j = 0; j < terrain.patchobjectnum[whichpatchx][whichpatchz]; j++) {
                                ...
                                if(!spritehit)
                                    if(objects.model[k].LineCheck(
                                           &start, &end, &colpoint, &objects.position[k], &objects.rotation[k]) != -1) {
                                        if(config.video_detail == 2 || (config.video_detail == 1 && abs(rand() % 4) == 0) ||
                                           (config.video_detail == 0 && abs(rand() % 8) == 0))
                                            objects.model[k].MakeDecal(
                                                blooddecalfast,
                                                DoRotation(colpoint - objects.position[k], 0, -objects.rotation[k], 0),
                                                size[i] * 1.6 /*+abs((float)(rand()%100))/2400*/,
                                                .5,
                                                rand() % 360);
                                        DeleteSprite(i);
                                        spritehit = 1;
        ...
        if(type[i] == splintersprite) {
            ...
            opacity[i] -= multiplier / 2;
            if(special[i] == 0 || special[i] == 2 || special[i] == 3)
                velocity[i].y += gravity * multiplier;
            if(special[i] == 1)
                velocity[i].y += gravity * multiplier * .5;
        }
        if(type[i] == flamesprite || type[i] == weaponflamesprite || type[i] == weaponshinesprite ||
           type[i] == bloodflamesprite) {
            ...
            if(type[i] != weaponshinesprite && type[i] != bloodflamesprite)
                if(opacity[i] < .5 && opacity[i] + multiplier * 5 / 4 >= .5 &&
                   (abs(rand() % 4) == 0 || (initialsize[i] > 2 && rand() % 2 == 0)))
                    MakeSprite(smoketype, position[i], velocity[i], .9, .9, .6, size[i] * 1.2, .4);
...
void Sprites::MakeSprite(int atype,
                         XYZ where,
                         XYZ avelocity,
                         float red,
                         float green,
                         ...
                         float aopacity)
{
    if(numsprites < max_sprites - 1) {
        if((atype != bloodsprite && atype != bloodflamesprite) || config.video_blood) {
