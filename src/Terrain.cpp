    ...
    for(i = patch_size * whichx; i < patch_size * (whichx + 1) + 1; i += stepsize) {
        for(j = patch_size * whichy; j < patch_size * (whichy + 1) + 1; j += stepsize) {
            if(i < size && j < size) {
                ...
                colors[i][j][3] =
                    (viewdistsquared - (distance - (viewdistsquared * fadestart)) * (1 / (1 - fadestart))) /
                    viewdistsquared;
            }
        }
    }
...
void Terrain::UpdateVertexArray(int whichx, int whichy)
{
    ...
    for(i = patch_size * whichx; i < patch_size * (whichx + 1); i += stepsize) {
        for(j = patch_size * whichy; j < patch_size * (whichy + 1); j += stepsize) {
            ...
            vArray[(a * 54) + (b * 54 * patch_size / stepsize) + c + 16] =
                i * scale * texscale + texoffsetx[i][j + stepsize];
            vArray[(a * 54) + (b * 54 * patch_size / stepsize) + c + 17] =
                j * scale * texscale + stepsize * scale * texscale + texoffsety[i][j + stepsize];
            ...
            vArray[(a * 54) + (b * 54 * patch_size / stepsize) + c + 25] =
                i * scale * texscale + stepsize * scale * texscale + texoffsetx[i + stepsize][j];
            vArray[(a * 54) + (b * 54 * patch_size / stepsize) + c + 26] =
                j * scale * texscale + texoffsety[i + stepsize][j];
            ...
            vArray[(a * 54) + (b * 54 * patch_size / stepsize) + c + 34] =
                i * scale * texscale + stepsize * scale * texscale + texoffsetx[i + stepsize][j];
            vArray[(a * 54) + (b * 54 * patch_size / stepsize) + c + 35] =
                j * scale * texscale + texoffsety[i + stepsize][j];
            ...
            vArray[(a * 54) + (b * 54 * patch_size / stepsize) + c + 43] =
                i * scale * texscale + texoffsetx[i][j + stepsize];
            vArray[(a * 54) + (b * 54 * patch_size / stepsize) + c + 44] =
                j * scale * texscale + stepsize * scale * texscale + texoffsety[i][j + stepsize];

            vArray[(a * 54) + (b * 54 * patch_size / stepsize) + c + 45] = i * scale + stepsize * scale;
            vArray[(a * 54) + (b * 54 * patch_size / stepsize) + c + 46] =
                heightmap[i + stepsize][j + stepsize] * scale;
            ...
            vArray[(a * 54) + (b * 54 * patch_size / stepsize) + c + 52] =
                i * scale * texscale + stepsize * scale * texscale + texoffsetx[i + stepsize][j + stepsize];
            vArray[(a * 54) + (b * 54 * patch_size / stepsize) + c + 53] =
                j * scale * texscale + stepsize * scale * texscale + texoffsety[i + stepsize][j + stepsize];
    ...
    for(a = 0; a < size / subdivision; a++) {
        for(b = 0; b < size / subdivision; b++) {
            if(heightmap[(size / subdivision) * whichx + a][(size / subdivision) * whichy + b] * scale >
               maxypatch[whichx][whichy])
                maxypatch[whichx][whichy] =
                    heightmap[(size / subdivision) * whichx + a][(size / subdivision) * whichy + b] * scale;
            if(heightmap[(size / subdivision) * whichx + a][(size / subdivision) * whichy + b] * scale <
               minypatch[whichx][whichy])
                minypatch[whichx][whichy] =
                    heightmap[(size / subdivision) * whichx + a][(size / subdivision) * whichy + b] * scale;
        }
    }
...
bool Terrain::load(const char* filename)
{
    ...
    const std::string name = locateDataFile("models", filename);
        ...
        for(j = 0; j < size; j++) {
            ...
            if(i > 0) {
                ...
                if(opacityother[x][y] &&
                   textureness[(int)(i * subdivision / size)][(int)(j * subdivision / size)] == allfirst)
                    textureness[(int)(i * subdivision / size)][(int)(j * subdivision / size)] = mixed;
                if(opacityother[x][y] != 1 &&
                   textureness[(int)(i * subdivision / size)][(int)(j * subdivision / size)] == allsecond)
                    textureness[(int)(i * subdivision / size)][(int)(j * subdivision / size)] = mixed;
                if(opacityother[i][j] &&
                   textureness[(int)(x * subdivision / size)][(int)(y * subdivision / size)] == allfirst)
                    textureness[(int)(x * subdivision / size)][(int)(y * subdivision / size)] = mixed;
                if(opacityother[i][j] != 1 &&
                   textureness[(int)(x * subdivision / size)][(int)(y * subdivision / size)] == allsecond)
                    textureness[(int)(x * subdivision / size)][(int)(y * subdivision / size)] = mixed;
...
void Terrain::drawpatch(int whichx, int whichy, float opacity)
{
    ...
    glVertexPointer(
        3, GL_FLOAT, 9 * sizeof(GLfloat), &vArray[0 + whichx * patch_elements + whichy * patch_elements * subdivision]);
    glColorPointer(
        4, GL_FLOAT, 9 * sizeof(GLfloat), &vArray[3 + whichx * patch_elements + whichy * patch_elements * subdivision]);
    glTexCoordPointer(
        2, GL_FLOAT, 9 * sizeof(GLfloat), &vArray[7 + whichx * patch_elements + whichy * patch_elements * subdivision]);
...
float Terrain::getOpacity(float pointx, float pointz)
{
    ...
    height2 =
        opacityother[tilex][tiley + 1] * (1 - (pointx - tilex)) + opacityother[tilex + 1][tiley + 1] * (pointx - tilex);
...
XYZ Terrain::getLighting(float pointx, float pointz)
{
    ...
    height2.x =
        colors[tilex][tiley + 1][0] * (1 - (pointx - tilex)) + colors[tilex + 1][tiley + 1][0] * (pointx - tilex);
    height2.y =
        colors[tilex][tiley + 1][1] * (1 - (pointx - tilex)) + colors[tilex + 1][tiley + 1][1] * (pointx - tilex);
    height2.z =
        colors[tilex][tiley + 1][2] * (1 - (pointx - tilex)) + colors[tilex + 1][tiley + 1][2] * (pointx - tilex);
    ...
    for(i = beginx; i < endx; i++) {
        for(j = beginz; j < endz; j++) {
            if(distance[i][j] < (viewdistance + patch_size) * (viewdistance + patch_size)) {
                ...
                glPushMatrix();
                if(frustum.CubeInFrustum(i * patch_size + patch_size * .5,
                                         avgypatch[i][j],
                                         j * patch_size + patch_size * .5,
                                         heightypatch[i][j] / 2)) {
        ...
        for(i = 0; i < numdecals; i++) {
            ...
            if(decaltype[i] == shadowdecal || decaltype[i] == shadowdecalpermanent) {
                distancemult = (viewdistsquared -
                                (findDistancefast(&viewer, &decalposition[i]) - (viewdistsquared * fadestart)) *
                                    (1 / (1 - fadestart))) /
                               viewdistsquared;
                if(distancemult >= 1)
            ...
            if((decaltype[i] == blooddecal || decaltype[i] == blooddecalfast || decaltype[i] == blooddecalslow)) {
                ...
                if(distancemult >= 1) {
                    ...
                    if(decalalivetime[i] < 4)
                        glColor4f(decalbrightness[i],
                                  decalbrightness[i],
                                  decalbrightness[i],
                                  decalopacity[i] * decalalivetime[i] * .25);
                    if(decalalivetime[i] > 58)
                        glColor4f(decalbrightness[i],
                                  decalbrightness[i],
                                  decalbrightness[i],
                                  decalopacity[i] * (60 - decalalivetime[i]) / 2);
                }
                if(distancemult < 1) {
                    glColor4f(
                        decalbrightness[i], decalbrightness[i], decalbrightness[i], decalopacity[i] * distancemult);
                    if(decalalivetime[i] < 4)
                        glColor4f(decalbrightness[i],
                                  decalbrightness[i],
                                  decalbrightness[i],
                                  decalopacity[i] * decalalivetime[i] * distancemult * .25);
                    if(decalalivetime[i] > 58)
                        glColor4f(decalbrightness[i],
                                  decalbrightness[i],
                                  decalbrightness[i],
                                  decalopacity[i] * (60 - decalalivetime[i]) / 2 * distancemult);
                }
            }
        ...
        for(i = 0; i < subdivision; i++) {
            for(j = 0; j < subdivision; j++) {
                if(patchobjectnum[i][j] < 300 - 1) {
                    ...
                    if(!done && where.x + radius > points[0].x && where.x - radius < points[2].x &&
                       where.z + radius > points[0].z && where.z - radius < points[2].z) {
...
void Terrain::MakeDecal(int type, XYZ where, float size, float opacity, float rotation)
{
    if(config.video_decals) {
        if(opacity > 0 && size > 0) {
            ...
            */
            if((patchx[0] != patchx[1] || patchy[0] != patchy[1]) &&
               (patchx[0] != patchx[2] || patchy[0] != patchy[2]) &&
               (patchx[0] != patchx[3] || patchy[0] != patchy[3])) {
                MakeDecalLock(type, where, patchx[0], patchy[0], size, opacity, rotation);
            }

            if((patchx[1] != patchx[2] || patchy[1] != patchy[2]) &&
               (patchx[1] != patchx[3] || patchy[1] != patchy[3])) {
        ...
        if(!(decaltexcoords[numdecals][0][0] < 0 && decaltexcoords[numdecals][1][0] < 0 &&
            ...
            if(!(decaltexcoords[numdecals][0][1] < 0 && decaltexcoords[numdecals][1][1] < 0 &&
                ...
                if(!(decaltexcoords[numdecals][0][0] > 1 && decaltexcoords[numdecals][1][0] > 1 &&
                    ...
                    if(!(decaltexcoords[numdecals][0][1] > 1 && decaltexcoords[numdecals][1][1] > 1 &&
            ...
            if(patchobjectnum[patchx][patchz]) {
                for(k = 0; k < patchobjectnum[patchx][patchz]; k++) {
                    ...
                    if(objects.type[l] != treetrunktype) {
                        ...
                        if(objects.model[l].LineCheck(
                               &testpoint, &testpoint2, &col, &objects.position[l], &objects.rotation[l]) != -1) {
