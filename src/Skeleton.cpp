...
void Skeleton::FindForwardsfirst()
{
    ...
    CrossProduct(joints[forwardjoints[1]].position - joints[forwardjoints[0]].position,
                 joints[forwardjoints[2]].position - joints[forwardjoints[0]].position,
                 &forward);
    Normalise(&forward);

    CrossProduct(joints[lowforwardjoints[1]].position - joints[lowforwardjoints[0]].position,
                 joints[lowforwardjoints[2]].position - joints[lowforwardjoints[0]].position,
                 &lowforward);
    ...
    if(free) {
        ...
        for(j = 0; j < numrepeats; j++) {
            if(!joints[jointlabels[rightknee]].locked && !joints[jointlabels[righthip]].locked) {
                temp = joints[jointlabels[rightknee]].position -
                       (joints[jointlabels[righthip]].position + joints[jointlabels[rightankle]].position) / 2;
                while(normaldotproduct(temp, lowforward) > -.1 &&
                      !sphere_line_intersection(&joints[jointlabels[righthip]].position,
                                                &joints[jointlabels[rightankle]].position,
                                                &joints[jointlabels[rightknee]].position,
                                                &r)) {
            ...
            if(!joints[jointlabels[leftknee]].locked && !joints[jointlabels[righthip]].locked) {
                temp = joints[jointlabels[leftknee]].position -
                       (joints[jointlabels[lefthip]].position + joints[jointlabels[leftankle]].position) / 2;
                while(normaldotproduct(temp, lowforward) > -.1 &&
                      !sphere_line_intersection(&joints[jointlabels[lefthip]].position,
                                                &joints[jointlabels[leftankle]].position,
                                                &joints[jointlabels[leftknee]].position,
                                                &r)) {
            ...
            sprites.MakeSprite(cloudsprite, headpos*(*scale)+*coords,joints[jointlabels[head]].velocity*.06, 1,1,1, 4,
            .2);
            breaking=0;
            camerashake+=.6;
            ...
            if(findLengthfast(&bounceness)>500)sprites.MakeSprite(cloudsprite,
            headpos*(*scale)+*coords,joints[jointlabels[head]].velocity*.06, 1,1,1, .5, .2);
            ...
            */
            ...
            for(i = 0; i < num_joints; i++) {
                ...
                // Ground constraint
                groundlevel = 0;
                if(joints[i].position.y * (*scale) + coords->y <
                   terrain.getHeight(joints[i].position.x * (*scale) + coords->x,
                                     joints[i].position.z * (*scale) + coords->z) +
                       groundlevel) {
                    freefall = 0;
                    friction = 1.5;
                    ...
                    terrainnormal = terrain.getNormal(joints[i].position.x * (*scale) + coords->x,
                                                      joints[i].position.z * (*scale) + coords->z);
                    ReflectVector(&joints[i].velocity, &terrainnormal);
                    bounceness = terrainnormal * findLength(&joints[i].velocity) *
                                 (abs(normaldotproduct(joints[i].velocity, terrainnormal)));
                    if(!joints[i].locked)
                        damage += findLengthfast(&bounceness) / 4000;
                    if(findLengthfast(&joints[i].velocity) < findLengthfast(&bounceness))
                        bounceness = 0;
                    frictionness = abs(normaldotproduct(
                        joints[i].velocity, terrainnormal)); // findLength(&bounceness)/findLength(&joints[i].velocity);
                    ...
                    if(tutoriallevel != 1 || id == 0)
                        if(findLengthfast(&bounceness) > 8000 && breaking) {
                            objects.model[k].MakeDecal(
                                breakdecal,
                                DoRotation(temp - objects.position[k], 0, -objects.rotation[k], 0),
                                .4,
                                .5,
                                rand() % 360);
                            sprites.MakeSprite(cloudsprite,
                                               joints[i].position * (*scale) + *coords,
                                               joints[i].velocity * .06,
                                               1,
                                               1,
                                               1,
                                               4,
                                               .2);
                            // sprites.MakeSprite(cloudsprite,
                            // joints[i].position*(*scale)+*coords,joints[i].velocity*.06, 1,1,1, 1, .2);
                            breaking = 0;
                            camerashake += .6;
                    ...
                    if(environment == snowyenvironment && findLengthfast(&bounceness) > 500 &&
                       terrain.getOpacity(joints[i].position.x * (*scale) + coords->x,
                                          joints[i].position.z * (*scale) + coords->z) < .2) {
                        terrainlight = terrain.getLighting(joints[i].position.x * (*scale) + coords->x,
                                                           joints[i].position.z * (*scale) + coords->z);
                        sprites.MakeSprite(cloudsprite,
                                           joints[i].position * (*scale) + *coords,
                                           joints[i].velocity * .06,
                    ...
                    } else if(environment == desertenvironment && findLengthfast(&bounceness) > 500 &&
                              terrain.getOpacity(joints[i].position.x * (*scale) + coords->x,
                                                 joints[i].position.z * (*scale) + coords->z) < .2) {
                        terrainlight = terrain.getLighting(joints[i].position.x * (*scale) + coords->x,
                                                           joints[i].position.z * (*scale) + coords->z);
                    ...
                    else if(environment == grassyenvironment && findLengthfast(&bounceness) > 500 &&
                            terrain.getOpacity(joints[i].position.x * (*scale) + coords->x,
                                               joints[i].position.z * (*scale) + coords->z) < .2) {
                        terrainlight = terrain.getLighting(joints[i].position.x * (*scale) + coords->x,
                                                           joints[i].position.z * (*scale) + coords->z);
                    ...
                    joints[i].position.y = (terrain.getHeight(joints[i].position.x * (*scale) + coords->x,
                                                              joints[i].position.z * (*scale) + coords->z) +
                                            groundlevel - coords->y) /
                                           (*scale);
                    if(longdead > 100)
                        broken = 1;
                }
                    ...
                    for(m = 0; m < terrain.patchobjectnum[whichpatchx][whichpatchz]; m++) {
                        ...
                        if(k < objects.numobjects && k >= 0)
                            if(objects.possible[k]) {
                                ...
                                whichhit = objects.model[k].LineCheckPossible(
                                    &start, &end, &temp, &objects.position[k], &objects.rotation[k]);
                                if(whichhit != -1) {
                                    if(joints[i].label == groin && !joints[i].locked && joints[i].delay <= 0) {
                                        joints[i].locked = 1;
                                        joints[i].delay = 1;
                                    ...
                                    if(objects.type[k] == treetrunktype) {
                                        ...
                                        sprites.MakeSprite(splintersprite, pos,tempvel*.5,
                                        165/255+float(abs(rand()%100)-50)/400,0,0, .2+float(abs(rand()%100)-50)/1300,
                                        1);
                                    ...
                                    frictionness = abs(normaldotproduct(
                                        joints[i].velocity,
                                        terrainnormal)); // findLength(&bounceness)/findLength(&joints[i].velocity);
                                    joints[i].velocity -= bounceness;
                                    if(1 - friction * frictionness > 0)
                                        joints[i].velocity *= 1 - friction * frictionness;
                                    else
            ...
            for(m = 0; m < terrain.patchobjectnum[whichpatchx][whichpatchz]; m++) {
                ...
                if(objects.possible[k]) {
                    for(i = 0; i < 26; i++) {
                        ...
                        whichhit = objects.model[k].LineCheckSlidePossible(
                            &start, &end, &temp, &objects.position[k], &objects.rotation[k]);
                        if(whichhit != -1) {
                            joints[jointlabels[whichjointendarray[i]]].position = (end - *coords) / (*scale);
                            for(j = 0; j < num_muscles; j++) {
                                if((muscles[j].parent1->label == whichjointstartarray[i] &&
                                    muscles[j].parent2->label == whichjointendarray[i]) ||
                                   (muscles[j].parent2->label == whichjointstartarray[i] &&
                                    muscles[j].parent1->label == whichjointendarray[i]))
        ...
        for(i = 0; i < num_joints; i++) {
            ...
            joints[i].position.y += groundlevel;
            joints[i].mass = 1;
            if(joints[i].label == lefthip || joints[i].label == leftknee || joints[i].label == leftankle ||
               joints[i].label == righthip || joints[i].label == rightknee || joints[i].label == rightankle)
                joints[i].mass = 2;
            if(joints[i].locked) {
                joints[i].mass = 4;
            }
...
void Skeleton::DoGravity(float* scale)
{
    static int i;
    for(i = 0; i < num_joints; i++) {
        if(((joints[i].label != leftknee && joints[i].label != rightknee) || lowforward.y > -.1 ||
            joints[i].mass < 5) &&
           ((joints[i].label != rightelbow && joints[i].label != rightelbow) || forward.y < .3))
...
void Skeleton::Draw(int muscleview)
{
    ...
    for(int i = 0; i < num_joints; i++) {
        if(joints[i].hasparent) {
            ...
            glColor4f(jointcolor[0], jointcolor[1], jointcolor[2], jointcolor[3] / joints[i].parent->blurred);
            glVertex3f(
                joints[i].parent->oldposition.x, joints[i].parent->oldposition.y, joints[i].parent->oldposition.z);
            glColor4f(jointcolor[0], jointcolor[1], jointcolor[2], jointcolor[3] / joints[i].blurred);
    ...
    if(muscleview == 1)
        for(int i = 0; i < num_muscles; i++) {
            if(muscles[i].type != boneconnect) {
                glVertex3f(
                    muscles[i].parent1->position.x, muscles[i].parent1->position.y, muscles[i].parent1->position.z);
                glVertex3f(
                    muscles[i].parent2->position.x, muscles[i].parent2->position.y, muscles[i].parent2->position.z);
            }
        }
    glEnd();
...
void Skeleton::DeleteJoint(int whichjoint)
{
    if(whichjoint < num_joints && whichjoint >= 0) {
        ...
        for(int i = 0; i < num_muscles; i++) {
            while(muscles[i].parent1 == &joints[num_joints - 1] && i < num_muscles)
                muscles[i].parent1 = &joints[whichjoint];
            while(muscles[i].parent2 == &joints[num_joints - 1] && i < num_muscles)
                muscles[i].parent2 = &joints[whichjoint];
        }
        for(int i = 0; i < num_joints; i++) {
    ...
    if(whichjoint < num_joints) {
        ...
        if(which < num_joints && which >= 0) {
            ...
            joints[whichjoint].length =
                findDistance(&joints[whichjoint].position, &joints[whichjoint].parent->position);
        }
    }
}
...
{
    if(num_muscles < max_muscles - 1 && attach1 < num_joints && attach1 >= 0 && attach2 < num_joints && attach2 >= 0 &&
       attach1 != attach2) {
        muscles[num_muscles].parent1 = &joints[attach1];
        muscles[num_muscles].parent2 = &joints[attach2];
        muscles[num_muscles].length =
            findDistance(&muscles[num_muscles].parent1->position, &muscles[num_muscles].parent2->position);
...
void Skeleton::FindRotationMuscle(int which, int animation)
{
    ...
    distance = sqrt((temppoint1.x - temppoint2.x) * (temppoint1.x - temppoint2.x) +
                    (temppoint1.y - temppoint2.y) * (temppoint1.y - temppoint2.y) +
                    (temppoint1.z - temppoint2.z) * (temppoint1.z - temppoint2.z));
    ...
    else if(muscles[which].parent1->label == rightshoulder || muscles[which].parent1->label == rightelbow ||
            muscles[which].parent1->label == rightwrist || muscles[which].parent1->label == righthand)
        tempforward = specialforward[1];
    else if(muscles[which].parent1->label == leftshoulder || muscles[which].parent1->label == leftelbow ||
            muscles[which].parent1->label == leftwrist || muscles[which].parent1->label == lefthand)
        tempforward = specialforward[2];
    else if(muscles[which].parent1->label == righthip || muscles[which].parent1->label == rightknee ||
            muscles[which].parent1->label == rightankle || muscles[which].parent1->label == rightfoot)
        tempforward = specialforward[3];
    else if(muscles[which].parent1->label == lefthip || muscles[which].parent1->label == leftknee ||
            muscles[which].parent1->label == leftankle || muscles[which].parent1->label == leftfoot)
...
void Animation::Load(const char* filename, int aheight, int aattack)
{
    ...
    LOG->LOG("Loading animation... %s", filename);

    std::string name = locateDataFile("animations", filename);
                    ...
                    bool aclothes)
{
    ...
    tfile = fopen(locateDataFile("textures", filename).c_str(), "rb");
    if(1) {
        ...
        for(i = 0; i < num_muscles; i++) {
            ...
            funpackf(tfile,
                     "Bf Bf Bf Bf Bf Bi Bi",
                     &muscles[i].length,
                     &muscles[i].targetlength,
                     &muscles[i].minlength,
                     &muscles[i].maxlength,
                     &muscles[i].strength,
                     &muscles[i].type,
        ...
        for(int k = 0; k < num_models; k++) {
            for(i = 0; i < model[k].vertexNum; i++) {
                model[k].vertex[i] =
                    model[k].vertex[i] -
                    (muscles[model[k].owner[i]].parent1->position + muscles[model[k].owner[i]].parent2->position) / 2;
    ...
    tfile = fopen(locateDataFile("textures", lowfilename).c_str(), "rb");
    if(1) {
        ...
        for(i = 0; i < modellow.vertexNum; i++) {
            modellow.vertex[i] =
                modellow.vertex[i] -
                (muscles[modellow.owner[i]].parent1->position + muscles[modellow.owner[i]].parent2->position) / 2;
            glMatrixMode(GL_MODELVIEW); // Select The Modelview Matrix
            glPushMatrix();
            glLoadIdentity();
    ...
    if(clothes) {
        tfile = fopen(locateDataFile("textures", clothesfilename).c_str(), "rb");
        lSize = sizeof(num_joints);
        fseek(tfile, lSize, SEEK_CUR);
        // joints = new Joint[num_joints];
        // jointlabels = new int[num_joints];
        ...
        for(i = 0; i < modelclothes.vertexNum; i++) {
            modelclothes.vertex[i] =
                modelclothes.vertex[i] -
                (muscles[modelclothes.owner[i]].parent1->position + muscles[modelclothes.owner[i]].parent2->position) /
                    2;
            glMatrixMode(GL_MODELVIEW); // Select The Modelview Matrix
            glPushMatrix();
            glLoadIdentity();
