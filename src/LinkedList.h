...
public:
    ...
    {
        if(locked == true)
            return false; // dont have to wait in this case
...
public:
    ...
    NMBoolean acquire()
    {
        int error = pthread_mutex_trylock(&theMutex);
        if(error)
            return false;
        else
            return true;
...
#define OTGetLinkObject(link, struc, field) ((struc*)((char*)(link) - offsetof(struc, field)))
